...
# ===================================================================
# Controls-specific fault-injection tests (CTRL-DELTA-05)
# ===================================================================


def _safe_set_threshold(
    driver: _MockDriver, session: _MockSession,
    threshold=None,
) -> dict:
    """Simulates an L4 handler for control.set_threshold with validation."""
    if threshold is None:
        return {
            "error": True,
            "code": "MISSING_THRESHOLD",
            "message": "threshold is required but missing",
            "safe": True,
        }
    ...
    if threshold < 0:
        return {
            "error": True,
            "code": "INVALID_VALUE",
            "message": f"threshold must be non-negative, got {threshold}",
            "safe": True,
        }
    return _safe_call(driver, session, threshold=threshold)


def _safe_killswitch_activate(
    driver: _MockDriver, session: _MockSession,
    entity_id: str | None = None, entity_frozen: bool = False,
) -> dict:
    """Simulates an L4 handler for killswitch.activate with validation."""
    if not entity_id:
        return {
            "error": True,
            "code": "MISSING_ENTITY",
            "message": "entity_id is required but missing",
            "safe": True,
        }
    if entity_frozen:
        return {
            "error": True,
            "code": "ALREADY_FROZEN",
            "message": "entity is already frozen; cannot double-freeze",
            "safe": True,
        }
    return _safe_call(driver, session, entity_id=entity_id)


def _safe_override_apply(
    driver: _MockDriver, session: _MockSession,
    limit_id: str | None = None, limit_exists: bool = True,
    override_value=None,
) -> dict:
    """Simulates an L4 handler for override.apply with validation."""
    if not limit_id:
        return {
            "error": True,
            "code": "MISSING_LIMIT",
            "message": "limit_id is required but missing",
            "safe": True,
        }
    if not limit_exists:
        return {
            "error": True,
            "code": "NOT_FOUND",
            "message": f"limit_id '{limit_id}' does not exist",
            "safe": True,
        }
    if override_value is None:
        return {
            "error": True,
            "code": "MISSING_VALUE",
            "message": "override_value is required but missing",
            "safe": True,
        }
    if not isinstance(override_value, (int, float)) or override_value < 0:
        return {
            "error": True,
            "code": "INVALID_VALUE",
            "message": f"override_value must be non-negative numeric, got {override_value}",
            "safe": True,
        }
    return _safe_call(driver, session, limit_id=limit_id, override_value=override_value)


class TestControlsFaultInjection:
    """CTRL-DELTA-05: Fault-injection tests for controls operations."""

    # CFI-001  Threshold driver timeout
    @pytest.mark.parametrize("exc", [TimeoutError("driver timeout")])
    def test_threshold_driver_timeout_returns_structured_error(self, exc):
        """control.set_threshold driver timeout → structured error."""
        driver = _MockDriver(side_effect=exc)
        session = _MockSession()

        result = _safe_set_threshold(driver, session, threshold=75.0)

        assert result["error"] is True
        assert result["safe"] is True

    # CFI-002  Threshold missing
    def test_threshold_missing_returns_structured_error(self):
        """control.set_threshold with no threshold → structured error."""
        driver = _MockDriver(return_value={"status": "ok"})
        session = _MockSession()

        result = _safe_set_threshold(driver, session, threshold=None)

        assert result["error"] is True
        assert "threshold" in result["message"]

    # CFI-003  Threshold negative
    def test_threshold_negative_returns_structured_error(self):
        """control.set_threshold with negative value → structured error."""
        driver = _MockDriver(return_value={"status": "ok"})
        session = _MockSession()

        result = _safe_set_threshold(driver, session, threshold=-10.0)

        assert result["error"] is True
        assert "-10" in result["message"]

    # CFI-004  Threshold non-numeric
    def test_threshold_non_numeric_returns_structured_error(self):
        """control.set_threshold with string value → structured error."""
        driver = _MockDriver(return_value={"status": "ok"})
        session = _MockSession()

        result = _safe_set_threshold(driver, session, threshold="high")

        assert result["error"] is True
        assert "numeric" in result["message"]

    # CFI-005  Killswitch missing entity
    def test_killswitch_missing_entity_returns_structured_error(self):
        """killswitch.activate with no entity_id → structured error."""
        driver = _MockDriver(return_value={"frozen": True})
        session = _MockSession()

        result = _safe_killswitch_activate(driver, session, entity_id=None)

        assert result["error"] is True
        assert "entity_id" in result["message"]

    # CFI-006  Killswitch already frozen
    def test_killswitch_already_frozen_returns_conflict(self):
        """killswitch.activate on frozen entity → structured error."""
        driver = _MockDriver(return_value={"frozen": True})
        session = _MockSession()

        result = _safe_killswitch_activate(
            driver, session, entity_id="e-001", entity_frozen=True
        )

        assert result["error"] is True
        assert "already frozen" in result["message"]

    # CFI-007  Override missing limit
    def test_override_missing_limit_returns_structured_error(self):
        """override.apply with no limit_id → structured error."""
        driver = _MockDriver(return_value={"status": "applied"})
        session = _MockSession()

        result = _safe_override_apply(driver, session, limit_id=None, override_value=100)

        assert result["error"] is True
        assert "limit_id" in result["message"]

    # CFI-008  Override non-existent limit
    def test_override_non_existent_limit_returns_not_found(self):
        """override.apply on non-existent limit → structured error."""
        driver = _MockDriver(return_value={"status": "applied"})
        session = _MockSession()

        result = _safe_override_apply(
            driver, session, limit_id="lim-001", limit_exists=False, override_value=100
        )

        assert result["error"] is True
        assert "does not exist" in result["message"]

    # CFI-009  Override connection refused
    def test_override_connection_refused_returns_structured_error(self):
        """override.apply with connection refused → structured error."""
        driver = _MockDriver(side_effect=ConnectionRefusedError("connection refused"))
        session = _MockSession()

        result = _safe_override_apply(
            driver, session, limit_id="lim-001", limit_exists=True, override_value=100
        )

        assert result["error"] is True
        assert result["safe"] is True

    # CFI-010  Threshold happy path
    def test_threshold_happy_path(self):
        """control.set_threshold with valid value → success."""
        driver = _MockDriver(return_value={"control_id": "ctrl-1", "threshold": 75.0})
        session = _MockSession()
    ...
    # CFI-011  Killswitch happy path
    def test_killswitch_activate_happy_path(self):
        """killswitch.activate on valid entity → success."""
        driver = _MockDriver(return_value={"entity_id": "e-001", "frozen": True})
        session = _MockSession()

        result = _safe_killswitch_activate(
            driver, session, entity_id="e-001", entity_frozen=False
        )

        assert result["error"] is False
        assert result["data"]["frozen"] is True

    # CFI-012  Override happy path
    def test_override_apply_happy_path(self):
        """override.apply with valid fields → success."""
        driver = _MockDriver(return_value={"limit_id": "lim-1", "status": "applied"})
        session = _MockSession()

        result = _safe_override_apply(
            driver, session, limit_id="lim-1", limit_exists=True, override_value=200
        )
