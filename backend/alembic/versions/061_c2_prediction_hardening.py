...
def upgrade() -> None:
    ...
    op.create_check_constraint("chk_prediction_advisory", "prediction_events", "is_advisory = TRUE")
    ...
    op.create_check_constraint(
        "chk_prediction_confidence_range", "prediction_events", "confidence_score >= 0 AND confidence_score <= 1"
    )

    # ============================================================
    # STEP 3: Rename valid_until to expires_at for clarity
    # ============================================================
    op.alter_column("prediction_events", "valid_until", new_column_name="expires_at")
    ...
    op.alter_column("prediction_events", "expires_at", nullable=False)

    # ============================================================
    # STEP 6: Add index for expiry-based cleanup
    # ============================================================
    # This supports the cleanup job
    op.create_index("ix_prediction_events_expires_at", "prediction_events", ["expires_at"])
...
def downgrade() -> None:
    ...
    op.alter_column("prediction_events", "expires_at", nullable=True)

    # Rename back to valid_until
    op.alter_column("prediction_events", "expires_at", new_column_name="valid_until")
