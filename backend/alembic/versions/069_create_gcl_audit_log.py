...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('object_type', sa.Text(), nullable=False),  # POLICY | INTEGRATION | SPEND_GUARD | KILLSWITCH | PREFERENCE
        ...
        sa.CheckConstraint(
            "actor_type IN ('HUMAN', 'SYSTEM_FACILITATION')",
            name='ck_gcl_audit_valid_actor_type'
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "confirmation = true",
            name='ck_gcl_audit_requires_confirmation'
        ),
    )

    # Indexes for replay queries per PIN-341 Section 3.2
    op.create_index('idx_gcl_audit_tenant_time', 'gcl_audit_log',
                    ['tenant_id', 'timestamp'])
    op.create_index('idx_gcl_audit_actor', 'gcl_audit_log',
                    ['actor_id', 'timestamp'])
    op.create_index('idx_gcl_audit_object', 'gcl_audit_log',
                    ['object_type', 'object_id', 'timestamp'])
    op.create_index('idx_gcl_audit_capability', 'gcl_audit_log',
                    ['capability_id', 'timestamp'])
    op.create_index('idx_gcl_audit_intent', 'gcl_audit_log',
                    ['intent', 'timestamp'])
    op.create_index('idx_gcl_audit_chain', 'gcl_audit_log',
    ...
    op.create_table(
        'gcl_replay_requests',
        sa.Column('replay_id', postgresql.UUID(as_uuid=True),
        ...
        sa.Column('filters', postgresql.JSONB(), nullable=False,
                  server_default=sa.text("'{}'::jsonb")),
        sa.Column('derived_view', postgresql.JSONB(), nullable=False,
                  server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_index('idx_gcl_replay_tenant', 'gcl_replay_requests',
                    ['tenant_id', 'requested_at'])
    op.create_index('idx_gcl_replay_actor', 'gcl_replay_requests',
