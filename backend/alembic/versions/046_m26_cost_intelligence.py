...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('id', sa.String(64), primary_key=True),
        sa.Column('tenant_id', sa.String(64), nullable=False, index=True),
        sa.Column('anomaly_type', sa.String(32), nullable=False),  # USER_SPIKE, FEATURE_SPIKE, BUDGET_WARNING, BUDGET_EXCEEDED
    ...
    op.create_table(
        ...
        sa.Column('entity_id', sa.String(128), nullable=True),  # null for tenant-level, feature_tag or user_id for entity-level
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('tenant_id', 'date', 'feature_tag', 'user_id', 'model',
                          name='uq_cost_daily_agg_composite'),
    )
