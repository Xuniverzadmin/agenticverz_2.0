...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('tenant_id', 'snapshot_type', 'period_start',
                          name='uq_cost_snapshots_tenant_type_period'),
    )
    # Index for finding latest complete snapshot
    op.create_index('ix_cost_snapshots_tenant_status', 'cost_snapshots',
                   ['tenant_id', 'status', 'period_end'])
    # Index for time-range queries
    op.create_index('ix_cost_snapshots_period', 'cost_snapshots',
    ...
    op.create_table(
        ...
        # Identity
        sa.Column('id', sa.String(64), primary_key=True),
        sa.Column('snapshot_id', sa.String(64), sa.ForeignKey('cost_snapshots.id', ondelete='CASCADE'),
        ...
        sa.UniqueConstraint('snapshot_id', 'entity_type', 'entity_id',
                          name='uq_cost_snapshot_agg_entity'),
    )
    # Index for entity lookups
    op.create_index('ix_cost_snapshot_agg_entity', 'cost_snapshot_aggregates',
                   ['tenant_id', 'entity_type', 'entity_id'])
    # Index for deviation queries (find anomalies)
    op.create_index('ix_cost_snapshot_agg_deviation', 'cost_snapshot_aggregates',
                   ['tenant_id', 'deviation_from_7d_pct'])
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('tenant_id', 'entity_type', 'entity_id', 'window_days', 'is_current',
                          name='uq_cost_baselines_current'),
    )
    # Index for finding current baselines
    op.create_index('ix_cost_baselines_current', 'cost_snapshot_baselines',
                   ['tenant_id', 'entity_type', 'entity_id', 'is_current'])
    ...
    op.create_table(
        ...
        sa.Column('id', sa.String(64), primary_key=True),
        sa.Column('tenant_id', sa.String(64), nullable=False, index=True),
        sa.Column('snapshot_id', sa.String(64), sa.ForeignKey('cost_snapshots.id', ondelete='SET NULL'),
    ...
    op.create_index('ix_cost_eval_tenant_time', 'cost_anomaly_evaluations',
                   ['tenant_id', 'evaluated_at'])
    op.create_index('ix_cost_eval_snapshot', 'cost_anomaly_evaluations',
    ...
    op.add_column('cost_anomalies',
                 sa.Column('baseline_id', sa.String(64), nullable=True))
