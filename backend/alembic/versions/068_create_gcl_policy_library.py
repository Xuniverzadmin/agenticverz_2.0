...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['parent_policy_id'], ['policy_library.policy_id'],
                                name='fk_policy_parent'),

        # Constraints per PIN-340
        sa.CheckConstraint(
            "policy_type IN ('RULE', 'LIMIT', 'SAFETY', 'COST', 'ACCESS')",
            name='ck_policy_library_valid_type'
        ),
        sa.CheckConstraint(
            "origin IN ('HUMAN', 'LEARNED', 'IMPORTED')",
            name='ck_policy_library_valid_origin'
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "scope IN ('ORG', 'PROJECT')",
            name='ck_policy_library_valid_scope'
        ),
        sa.CheckConstraint(
            "mode IN ('MONITOR', 'ENFORCE')",
            name='ck_policy_library_valid_mode'
        ),
    )

    # Indexes for policy_library
    op.create_index('idx_policy_library_tenant', 'policy_library', ['tenant_id'])
    op.create_index('idx_policy_library_project', 'policy_library', ['project_id'],
    ...
    op.create_index('idx_policy_library_active', 'policy_library',
                    ['tenant_id', 'status'],
    ...
    op.create_table(
        ...
        sa.Column('tenant_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('simulated_at', sa.TIMESTAMP(), nullable=False,
                  server_default=sa.text('NOW()')),
        ...
        # Foreign key
        sa.ForeignKeyConstraint(['policy_id'], ['policy_library.policy_id'],
                                name='fk_simulation_policy'),
    )
    ...
    op.create_index('idx_policy_sim_time', 'policy_simulation_results',
                    ['tenant_id', 'simulated_at'])
    ...
    op.create_table(
        ...
        # Foreign keys
        sa.ForeignKeyConstraint(['policy_id'], ['policy_library.policy_id'],
                                name='fk_activation_policy'),
        sa.ForeignKeyConstraint(['simulation_id'], ['policy_simulation_results.simulation_id'],
                                name='fk_activation_simulation'),
    ...
    op.create_index('idx_policy_activation_time', 'policy_activation_log',
                    ['policy_id', 'performed_at'])
