...
def upgrade() -> None:
    ...
    op.create_table(
        "signal_feedback",
        sa.Column(
            "feedback_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False
        ),
        ...
        sa.CheckConstraint("feedback IN ('USEFUL', 'NOT_USEFUL', 'IGNORED')", name="ck_signal_feedback_valid_feedback"),
        # One feedback per signal per actor per recommendation (PIN-344 2.5)
        sa.UniqueConstraint("signal_id", "recommendation_id", "actor_id", name="uq_signal_feedback_one_per_actor"),
    )

    # Indexes per PIN-344 Section 2.5
    op.create_index("idx_signal_feedback_tenant", "signal_feedback", ["tenant_id", "created_at"])
    op.create_index("idx_signal_feedback_signal", "signal_feedback", ["signal_type"])
    op.create_index("idx_signal_feedback_actor", "signal_feedback", ["actor_id", "created_at"])
    op.create_index(
        "idx_signal_feedback_useful",
        "signal_feedback",
    ...
    op.create_table(
        ...
        sa.Column("shown_to", postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=False, server_default="{}"),
        sa.Column("created_at", sa.TIMESTAMP(), nullable=False, server_default=sa.text("NOW()")),
        ...
        # Constraints
        sa.CheckConstraint(
            "severity IN ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')", name="ck_recommendation_valid_severity"
        ),
        sa.CheckConstraint("confidence >= 0.0 AND confidence <= 1.0", name="ck_recommendation_confidence_range"),
    )

    # Indexes
    op.create_index("idx_recommendations_tenant", "signal_recommendations", ["tenant_id", "created_at"])
    op.create_index("idx_recommendations_signal", "signal_recommendations", ["signal_type", "created_at"])
    ...
    op.create_index("idx_recommendations_expires", "signal_recommendations", ["tenant_id", "expires_at"])
