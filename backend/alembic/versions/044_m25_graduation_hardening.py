...
def upgrade():
    ...
    op.create_index('idx_timeline_views_real', 'timeline_views', ['viewed_at', 'has_prevention'], postgresql_where=sa.text('is_simulated = false'))
    ...
    op.add_column('m25_graduation_status',
        sa.Column('is_derived', sa.Boolean, server_default='true'))
    op.add_column('m25_graduation_status',
        sa.Column('last_evidence_eval', sa.DateTime(timezone=True)))
    op.add_column('m25_graduation_status',
        sa.Column('degraded_from', sa.String(32)))
    op.add_column('m25_graduation_status',
        sa.Column('degradation_reason', sa.Text))
