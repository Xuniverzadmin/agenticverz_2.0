...
"""
Policy Domain Facade

This facade provides the external interface to the Policy domain.
All code outside the Policy domain MUST use this facade.
...
logger = logging.getLogger("nova.services.policy.facade")

# Singleton instance
_policy_facade: Optional["PolicyFacade"] = None
...
class PolicyFacade:
    ...
    async def evaluate(self, request, db=None, dry_run: bool = False):
        ...
        """
        logger.debug("facade.evaluate", extra={"action_type": str(request.action_type)})
    ...
    async def pre_check(
        ...
        """
        logger.debug("facade.pre_check", extra={"agent_id": agent_id, "goal": goal})
    ...
    async def reload_policies(self, db=None):
        ...
        logger.info("facade.reload_policies")
...
def get_policy_facade(db_url: Optional[str] = None) -> PolicyFacade:
    """
    Get the PolicyFacade singleton.

    This is the ONLY way external code should access policy operations.
    Direct imports of PolicyEngine are forbidden per API-001.

    Args:
        db_url: Optional database URL override

    Returns:
        PolicyFacade singleton instance
    """
    global _policy_facade
    if _policy_facade is None:
        _policy_facade = PolicyFacade(db_url=db_url)
    return _policy_facade


def reset_policy_facade() -> None:
    """Reset the facade singleton (for testing)."""
    global _policy_facade
    _policy_facade = None
