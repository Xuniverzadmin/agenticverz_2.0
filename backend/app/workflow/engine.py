...
class WorkflowEngine:
    ...
    async def run(
        ...
        if ck and ck.step_outputs:
            ...
            logger.info(
                "workflow_resume",
                extra={"run_id": run_id, "from_step": start_index}
            )
            ...
            try:
                ...
                if hasattr(skill, 'invoke'):
                    result = await skill.invoke(
                        inputs=ctx.inputs,
                        seed=ctx.seed,
                        meta={"step_id": ctx.step_id, "attempt": attempt}
                    )
                ...
                if isinstance(result, dict):
                    if result.get("ok") is False or result.get("success") is False:
                        ...
                        error_code_str = error_info.get("code", "STEP_FAILED") if isinstance(error_info, dict) else "STEP_FAILED"
                        try:
                        ...
                        last_error = WorkflowError(
                            code=error_code,
                            message=error_info.get("message", "Step failed") if isinstance(error_info, dict) else str(error_info),
