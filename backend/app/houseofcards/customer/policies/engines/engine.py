...
#   M2. [DONE] _persist_evaluation()    - Authority: POLICY_EVALUATION_WRITE
#   M3. [DONE] get_violations()         - Authority: POLICY_VIOLATION_READ
#   M4. [DONE] get_violation()          - Authority: POLICY_VIOLATION_READ
#   M5. [DONE] acknowledge_violation()  - Authority: POLICY_VIOLATION_WRITE
#   M6. [DONE] update_risk_ceiling()    - Authority: POLICY_CEILING_WRITE
#   M7. [DONE] reset_risk_ceiling()     - Authority: POLICY_CEILING_WRITE
#   M8. [DONE] update_safety_rule()     - Authority: POLICY_RULE_WRITE
#   M9. [DONE] get_policy_versions()    - Authority: POLICY_VERSION_READ
#  M10. [DONE] get_current_version()    - Authority: POLICY_VERSION_READ
#  M11. [DONE] create_policy_version()  - Authority: POLICY_VERSION_WRITE
#  M12. [DONE] rollback_to_version()    - Authority: POLICY_VERSION_WRITE
#  M13. [DONE] get_version_provenance() - Authority: POLICY_PROVENANCE_READ
#  M14. [DONE] get_dependency_graph()   - Authority: POLICY_GRAPH_READ
#  M15. [DONE] get_policy_conflicts()   - Authority: POLICY_GRAPH_READ
#  M16. [DONE] resolve_conflict()       - Authority: POLICY_GRAPH_WRITE
#  M17. [DONE] get_temporal_policies()  - Authority: POLICY_TEMPORAL_READ
#  M18. [DONE] create_temporal_policy() - Authority: POLICY_TEMPORAL_WRITE
#  M19. [DONE] get_temporal_utilization() - Authority: POLICY_TEMPORAL_READ
#  M20. [DONE] validate_dependency_dag()  - Authority: POLICY_GRAPH_READ
#  M21. [DONE] add_dependency_with_dag_check() - Authority: POLICY_GRAPH_WRITE
#  M22. [DONE] prune_temporal_metrics()   - Authority: POLICY_TEMPORAL_WRITE
#  M23. [DONE] get_temporal_storage_stats() - Authority: POLICY_TEMPORAL_READ
#  M24. [DONE] activate_policy_version()  - Authority: POLICY_INTEGRITY_READ
#  M25. N/A    (extraction complete)      - Authority: N/A
#
# STOP CONDITIONS (Halt migration immediately if):
#   - Driver method requires conditional branching on policy type
#   - Driver method requires threshold/severity logic
#   - Driver method requires dependency graph traversal
#   - Two methods demand contradictory driver interfaces
#   - Engine method still "feels procedural" after extraction
#
# ============================================================================
...
# L6 driver import (allowed) - PIN-468
from app.houseofcards.customer.policies.drivers.policy_engine_driver import (
    PolicyEngineDriver,
...
class PolicyEngine:
    ...
    def __init__(self, database_url: Optional[str] = None, governor=None):
        ...
        # L6 driver for all DB operations (PIN-468)
        self._driver: Optional[PolicyEngineDriver] = None
        if self._db_url:
            self._driver = get_policy_engine_driver(self._db_url)
    ...
    async def _load_policies(self) -> PolicyLoadResult:
        ...
        M1 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms raw dicts → domain objects.
        """
        result = PolicyLoadResult()

        if not self._driver:
            # No driver = no database, load defaults
    ...
    async def _persist_evaluation(self, request: PolicyEvaluationRequest, result: PolicyEvaluationResult) -> None:
        ...
        M2 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms domain objects → primitive params for driver.
        """
        if not self._driver:
            return
    ...
    async def get_violations(
        ...
        M3 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms raw dicts → domain objects.
        """
        if not self._driver:
            return []
    ...
    async def get_violation(self, db, violation_id: str) -> Optional[PolicyViolation]:
        ...
        M4 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms raw dict → domain object.
        """
        if not self._driver:
            return None
    ...
    async def acknowledge_violation(self, db, violation_id: str, notes: Optional[str] = None) -> bool:
        ...
        M5 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
            return False
    ...
    async def update_risk_ceiling(self, db, ceiling_id: str, updates: Dict[str, Any]) -> Optional[RiskCeiling]:
        ...
        M6 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
            return None
    ...
    async def reset_risk_ceiling(self, db, ceiling_id: str) -> bool:
        ...
        M7 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
            return False
    ...
    async def update_safety_rule(self, db, rule_id: str, updates: Dict[str, Any]) -> Optional[SafetyRule]:
        ...
        M8 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms condition dict → JSON string before passing to driver.
        """
        if not self._driver:
            return None

        try:
            # Prepare updates - transform condition to JSON string
            driver_updates = {}
            for key, value in updates.items():
                if key == "condition":
                    driver_updates[key] = json.dumps(value)
                else:
    ...
    async def get_policy_versions(self, db=None, limit: int = 20, include_inactive: bool = False) -> List[Dict]:
        ...
        M9 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms raw dicts → formatted response dicts.
        """
        if not self._driver:
    ...
    async def get_current_version(self, db=None) -> Optional[Dict]:
        ...
        M10 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
            return {"version": self._policy_version, "is_active": True}
    ...
    async def create_policy_version(self, db, description: str, created_by: str = "system"):
        ...
        M11 Extraction (PIN-468): All DB access delegated to _driver.
        Engine handles version computation and policy hashing (domain logic).
        """
        import hashlib
        import uuid
        ...
        # Compute hash of current policies (domain logic - stays in engine)
        ...
        # Increment version (domain logic - stays in engine)
    ...
    async def rollback_to_version(self, db, target_version: str, reason: str, rolled_back_by: str):
        ...
        M12 Extraction (PIN-468): All DB access delegated to _driver.
        """
        import uuid
    ...
    async def get_version_provenance(self, db, version_id: str) -> List[Dict]:
        ...
        M13 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
            return []
    ...
    async def get_dependency_graph(self, db=None):
        ...
        M14 Extraction (PIN-468): All DB access delegated to _driver.
        Engine transforms raw dicts → domain objects.
        """
    ...
    async def get_policy_conflicts(self, db=None, include_resolved: bool = False) -> List:
        ...
        M15 Extraction (PIN-468): All DB access delegated to _driver.
        """
    ...
    async def resolve_conflict(self, db, conflict_id: str, resolution: str, resolved_by: str) -> bool:
        ...
        M16 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
            return False
    ...
    async def get_temporal_policies(
        ...
        M17 Extraction (PIN-468): All DB access delegated to _driver.
        """
    ...
    async def create_temporal_policy(self, db, data: Dict):
        ...
        M18 Extraction (PIN-468): All DB access delegated to _driver.
        """
        from app.policy.models import TemporalPolicy
    ...
    async def get_temporal_utilization(self, db, policy_id: str, agent_id: Optional[str] = None) -> Dict:
        ...
        M19 Extraction (PIN-468): All DB access delegated to _driver.
        """
        if not self._driver:
    ...
    async def validate_dependency_dag(self, db=None) -> Dict[str, Any]:
        ...
        M20 Extraction (PIN-468): DB access delegated to _driver.

        Returns:
            Dict with 'is_dag', 'cycles' (if any), and 'topological_order'
        """
        # Build adjacency list from dependencies
        graph: Dict[str, List[Tuple[str, str]]] = {}  # source -> [(target, dep_type)]
        all_nodes: set = set()
    ...
    async def add_dependency_with_dag_check(
        ...
        M21 Extraction (PIN-468): DB access delegated to _driver.
    ...
    async def prune_temporal_metrics(
        ...
        M22 Extraction (PIN-468): DB access delegated to _driver.
    ...
    async def get_temporal_storage_stats(self, db=None) -> Dict[str, Any]:
        ...
        M23 Extraction (PIN-468): DB access delegated to _driver.
        """
        if not self._driver:
    ...
    async def activate_policy_version(
        ...
        M24 Extraction (PIN-468): DB access delegated to _driver.
