...
class PlannerConfig(BaseModel):
    ...
    """
    planner_type: PlannerType = Field(
        default=PlannerType.STUB,
        description="Which planner backend to use"
    )
    ...
    max_tokens: int = Field(
        ...
        ge=256,
        le=8192,
        description="Max tokens for plan generation"
    )
    max_steps: int = Field(
        default=10,
        ge=1,
        le=50,
        description="Maximum steps in a plan"
    )
    system_prompt: Optional[str] = Field(
        default=None,
        description="Custom system prompt for planner"
    )
...
class RateLimitConfig(BaseModel):
    ...
    tokens_per_minute: Optional[int] = Field(
        ...
        ge=1000,
        le=1_000_000,
        description="Maximum LLM tokens per minute"
    )
    concurrent_runs: int = Field(
        default=5,
        ge=1,
        le=100,
        description="Maximum concurrent run executions"
    )
...
class BudgetConfig(BaseModel):
    ...
    budget_cents: Optional[int] = Field(
        default=None,
        ge=0,
        description="Total budget in cents (None = unlimited)"
    )
    spent_cents: int = Field(
        default=0,
        ge=0,
    ...
    hard_limit: bool = Field(
        default=True,
        description="Stop execution when budget exhausted"
    )
...
class AgentCapabilities(BaseModel):
    ...
    allowed_skills: Optional[List[str]] = Field(
        default=None,
        description="Whitelist of allowed skills (None = all allowed)"
    )
    blocked_skills: List[str] = Field(
        default_factory=list,
        description="Blacklist of blocked skills"
    )

    # Network/external access
    allow_external_http: bool = Field(
        default=True,
        description="Allow HTTP calls to external URLs"
    )
    allowed_domains: Optional[List[str]] = Field(
        default=None,
        description="Whitelist of allowed domains for HTTP (None = all)"
    )
    ...
    allowed_paths: Optional[List[str]] = Field(
        default=None,
        description="Whitelist of allowed file paths (glob patterns)"
    )
    ...
    allow_llm: bool = Field(
        default=True,
        description="Allow LLM invocations"
    )
...
class AgentConfig(BaseModel):
    ...
    default_retry_policy: RetryPolicy = Field(
        ...
        description="Default retry policy for runs"
    )

    # Ownership and access
    owner_id: Optional[str] = Field(
        default=None,
        description="Owner user/tenant ID"
    )
