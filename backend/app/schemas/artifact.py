...
class Artifact(BaseModel):
    ...
    storage_backend: StorageBackend = Field(
        default=StorageBackend.INLINE,
        description="Where artifact is stored"
    )
    storage_path: Optional[str] = Field(
        default=None,
        description="Path/key in storage backend"
    )

    # Content (for inline storage)
    content_json: Optional[Dict[str, Any]] = Field(
        default=None,
        description="JSON content (for JSON artifacts)"
    )
    content_text: Optional[str] = Field(
        default=None,
        max_length=100000,
        description="Text content (for text/html/markdown)"
    )
    ...
    encoding: Optional[str] = Field(
        default=None,
        description="Text encoding if applicable"
    )
    ...
    expires_at: Optional[datetime] = Field(
        default=None,
        description="Expiration timestamp (for cleanup)"
    )
    ...
    def has_content(self) -> bool:
        ...
        return (
            self.content_json is not None or
            self.content_text is not None or
