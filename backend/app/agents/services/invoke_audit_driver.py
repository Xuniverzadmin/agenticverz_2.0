...
# Role: Invoke audit data access driver
# Callers: agent_invoke.py
# Allowed Imports: sqlalchemy (self-managed connection)
# Forbidden Imports: L1, L2, L3, L4, L5
# Reference: PIN-468 (Phase 2 Step 2 - L4/L6 Segregation)
#
# GOVERNANCE NOTE:
# This L6 driver provides pure data access for invoke audit.
# Self-managed connection pool for operational isolation.
# NO business logic - only DB operations.
#
# RECLASSIFICATION: 2026-01-23
# Previously: invoke_audit_service.py (mislabeled as service)
# Reason: 0 decisions, 100% persistence = pure driver

"""
Invoke Audit Driver (L6)

Pure database operations for agent invoke audit trail.
Self-managed connection pool for operational isolation.
...
class InvokeAuditDriver:
    """
    L6 driver for invoke audit operations.

    Pure database access with self-managed connection pool.
    """

    def __init__(self, database_url: Optional[str] = None):
        self.database_url = database_url if database_url is not None else os.environ.get("DATABASE_URL")
        if not self.database_url:
            raise RuntimeError("DATABASE_URL required for InvokeAuditDriver")
...
# Singleton instance
_driver: Optional[InvokeAuditDriver] = None


def get_invoke_audit_driver() -> InvokeAuditDriver:
    """Get singleton invoke audit driver instance."""
    global _driver
    if _driver is None:
        _driver = InvokeAuditDriver()
    return _driver


# Backward compatibility aliases (DEPRECATED - remove after migration)
InvokeAuditService = InvokeAuditDriver
get_invoke_audit_service = get_invoke_audit_driver
