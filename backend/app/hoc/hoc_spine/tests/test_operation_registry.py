...
#
# HOC Traceability:
#   Subject under test: backend/app/hoc/hoc_spine/orchestrator/operation_registry.py
...
#   orchestrator/__init__.py has broken cross-domain imports (pending L1 re-wiring).
#   Placing tests inside orchestrator/ triggers the broken __init__.py chain.
#   This directory avoids that while keeping tests under hoc/.
#
# Run:
#   cd backend && PYTHONPATH=. python3 -m pytest app/hoc/hoc_spine/tests/test_operation_registry.py -v

"""
Operation Registry Tests

Tests for the L4 operation dispatch registry.
Subject: backend/app/hoc/hoc_spine/orchestrator/operation_registry.py
...
# Direct file import — avoids orchestrator/__init__.py which has broken
# cross-domain re-exports (ValidatorVerdict relocated, pending L1 re-wiring).
# Once __init__.py is cleaned up, switch to:
