...
class CostTracker:
    ...
    def get_spend(self, tenant_id: str, period: str = "daily") -> float:
    ...
    def get_remaining_budget(self, tenant_id: str, period: str = "daily") -> float:
    ...
    def is_budget_exceeded(self, tenant_id: str, period: str = "daily") -> bool:
        """Check if budget is exceeded."""
        return self.get_remaining_budget(tenant_id, period) <= 0

    def get_alerts(self, tenant_id: Optional[str] = None, since: Optional[datetime] = None) -> list[CostAlert]:
    ...
    def get_cost_summary(self, tenant_id: str, days: int = 7) -> dict:
        """Get cost summary for tenant."""
        now = datetime.now(timezone.utc)
        cutoff = now - timedelta(days=days)

        with self._lock:
            records = [r for r in self._records if r.tenant_id == tenant_id and r.timestamp >= cutoff]
    ...
    def check_can_spend(
        self, tenant_id: str, estimated_cost_cents: float, workflow_id: Optional[str] = None
        ...
        if workflow_id:
            ...
            if workflow_spend + estimated_cost_cents > self.quota.per_workflow_limit_cents:
                if self.quota.enforce_hard_limit:
                    ...
                    return (CostEnforcementResult.BUDGET_WARNING, "Workflow budget warning: approaching limit")
        ...
        if hourly_spend + estimated_cost_cents > self.quota.hourly_limit_cents:
            if self.quota.enforce_hard_limit:
                ...
                return (CostEnforcementResult.BUDGET_WARNING, "Hourly budget warning: approaching limit")
        ...
        if daily_spend + estimated_cost_cents > self.quota.daily_limit_cents:
            if self.quota.enforce_hard_limit:
                ...
                return (CostEnforcementResult.BUDGET_WARNING, "Daily budget warning: approaching limit")
    ...
    def get_workflow_spend(self, workflow_id: str) -> float:
        ...
        with self._lock:
            return sum(r.cost_cents for r in self._records if r.workflow_id == workflow_id)

    def set_workflow_budget(self, workflow_id: str, budget_cents: int) -> None:
