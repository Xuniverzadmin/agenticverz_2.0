...
class HttpCallInput(SkillInputBase):
    ...
    url: str = Field(description="URL to call", examples=["https://api.github.com/zen"])
    ...
    body: Optional[Union[Dict[str, Any], str]] = Field(default=None, description="Request body (JSON object or string)")
    timeout_seconds: float = Field(default=30.0, ge=0.1, le=300, description="Request timeout in seconds")
...
class HttpCallOutput(SkillOutputBase):
    ...
    response_body: Optional[str] = Field(
        default=None, max_length=10000, description="Response body (truncated to 10KB)"
    )
...
class LLMInvokeInput(SkillInputBase):
    ...
    enable_cache: bool = Field(default=True, description="Enable prompt caching for cost savings (default: True)")
...
class LLMInvokeOutput(SkillOutputBase):
    ...
    cost_cents: Optional[float] = Field(default=None, ge=0, description="Estimated cost in cents")
...
class FileReadInput(SkillInputBase):
    ...
    max_bytes: int = Field(default=1_000_000, ge=1, le=10_000_000, description="Maximum bytes to read")
...
class FileWriteInput(SkillInputBase):
    ...
    encoding: str = Field(default="utf-8", description="File encoding")
    mode: str = Field(default="write", pattern="^(write|append)$", description="Write mode: write or append")
...
class PostgresQueryInput(SkillInputBase):
    ...
    params: Optional[Dict[str, Any]] = Field(default=None, description="Query parameters for safe interpolation")
    database: Optional[str] = Field(default=None, description="Database name (uses default if not specified)")
    timeout_seconds: float = Field(default=30.0, ge=1, le=300, description="Query timeout")
...
class JsonTransformInput(SkillInputBase):
    ...
    jmespath: Optional[str] = Field(default=None, description="JMESPath expression to apply")
    jsonpath: Optional[str] = Field(default=None, description="JSONPath expression to apply")
    template: Optional[str] = Field(default=None, description="Jinja2 template for transformation")


class JsonTransformOutput(SkillOutputBase):
    """Output schema for json_transform skill."""

    result: Union[Dict[str, Any], List[Any], str, int, float, bool, None] = Field(description="Transformed result")
...
class EmailSendInput(SkillInputBase):
    ...
    from_address: Optional[str] = Field(default=None, description="Sender email (defaults to configured address)")
    ...
    tags: Optional[Dict[str, str]] = Field(default=None, description="Custom tags for tracking")
...
class EmailSendOutput(SkillOutputBase):
    ...
    message_id: Optional[str] = Field(default=None, description="Unique message ID from email provider")
    recipients: List[str] = Field(description="List of recipients email was sent to")
    accepted: int = Field(default=0, ge=0, description="Number of accepted recipients")
    rejected: int = Field(default=0, ge=0, description="Number of rejected recipients")
...
class KVStoreInput(SkillInputBase):
    ...
    value: Optional[Any] = Field(default=None, description="Value to set (for SET operation)")
    ...
    namespace: str = Field(default="default", max_length=64, description="Namespace for key isolation")
    idempotency_key: Optional[str] = Field(default=None, description="Idempotency key for SET/DELETE operations")
...
class SlackSendInput(SkillInputBase):
    ...
    webhook_url: Optional[str] = Field(default=None, description="Slack webhook URL (uses default if not set)")
    blocks: Optional[List[Dict[str, Any]]] = Field(default=None, description="Slack Block Kit blocks")
    attachments: Optional[List[Dict[str, Any]]] = Field(default=None, description="Legacy attachments")
    channel: Optional[str] = Field(default=None, description="Channel name (for display/logging)")
    username: Optional[str] = Field(default=None, description="Bot username override")
    icon_emoji: Optional[str] = Field(default=None, description="Bot icon emoji override")
    ...
    idempotency_key: Optional[str] = Field(default=None, description="Idempotency key to prevent duplicate posts")
...
class WebhookSendInput(SkillInputBase):
    ...
    sign_payload: bool = Field(default=True, description="Sign payload with HMAC-SHA256")
    signature_header: str = Field(default="X-Signature-256", description="Header name for signature")
    timestamp_header: str = Field(default="X-Signature-Timestamp", description="Header name for timestamp")
    timeout_seconds: float = Field(default=30.0, ge=1, le=120, description="Request timeout")
...
class WebhookSendOutput(SkillOutputBase):
    ...
    status_code: int = Field(description="HTTP response status code")
    response_body: Optional[str] = Field(default=None, max_length=10000, description="Response body (truncated)")
    request_id: Optional[str] = Field(default=None, description="Request ID from response headers")
    signature_sent: bool = Field(default=False, description="Whether signature was sent")
    from_cache: bool = Field(default=False, description="Result from idempotency cache")
...
class VoyageEmbedInput(SkillInputBase):
    ...
    model: VoyageModel = Field(default=VoyageModel.VOYAGE_3, description="Voyage model to use")
    input_type: Optional[VoyageInputType] = Field(
        default=None, description="Input type hint for retrieval optimization"
    )
    truncation: bool = Field(default=True, description="Truncate input if exceeds max tokens")
...
class SkillMetadata(BaseModel):
    ...
    requires_config: bool = Field(default=False, description="Whether skill requires configuration")
    config_schema: Optional[Dict[str, Any]] = Field(default=None, description="JSON Schema for config if required")
