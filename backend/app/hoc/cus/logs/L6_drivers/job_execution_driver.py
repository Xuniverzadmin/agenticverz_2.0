...
# NOTE: Renamed job_execution.py → job_execution_driver.py (2026-01-31)
# AUDIENCE: CUSTOMER
# Temporal:
...
# This file implements EXECUTION MECHANICS for retry, progress tracking, and audit emission.
# It does NOT contain business decisions. The retry strategies (exponential, fibonacci, etc.)
# are TECHNICAL POLICIES that define HOW retries are computed, not WHETHER to retry.
# The decision "should this job retry?" is made by L5 engines using RetryConfig from callers.
# This L6 driver only EXECUTES the timing math and records the attempt history.
# Layer L6 compliance: Pure execution mechanics, no policy evaluation.
