...
# Role: Customer logs boundary adapter (L2 → L4)
...
#
# This adapter promotes LOGS_LIST capability from L4 to adapter.

"""
Customer Logs Adapter (L2)

This adapter sits between L2 (guard_logs.py API) and L4 (LogsReadService).

L2 (Guard API) → L4 (LogsReadService)
...
class CustomerLogsAdapter:
    ...
    async def get_log(
        ...
        # L2 → L4 delegation with tenant enforcement
        service = await self._get_service()
