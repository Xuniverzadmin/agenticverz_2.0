...
class DataIngestionExecutor:
    ...
    def __init__(
        ...
        Args:
            ...
            connector_lookup: Injected ConnectorLookupPort (PIN-513 L1 re-wiring).
        """
    ...
    async def _get_connector(
        ...
        try:
            if self._connector_lookup is None:
                raise NotImplementedError(
                    "Connector lookup not injected. Wire via DataIngestionExecutor(connector_lookup=...)"
                )
            return self._connector_lookup.get_connector(connector_id, tenant_id)
        except Exception as e:
            logger.warning(
...
class IndexingExecutor:
    ...
    async def _get_vector_connector(
        ...
        try:
            if self._connector_lookup is None:
                raise NotImplementedError(
                    "Connector lookup not injected. Wire via IndexingExecutor(connector_lookup=...)"
                )
            return self._connector_lookup.get_connector(connector_id, tenant_id)
        except Exception as e:
            logger.warning(
