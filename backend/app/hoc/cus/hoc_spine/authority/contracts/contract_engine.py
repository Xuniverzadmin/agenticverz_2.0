...
# NOTE: Renamed contract_service.py → contract_engine.py (2026-01-24)
...
# PIN-513 L1 Re-wiring: Cross-domain types injected via Protocol interfaces.
# Callers pass objects that satisfy ValidatorVerdictPort / EligibilityVerdictPort.
# The actual types (ValidatorVerdict, EligibilityVerdict, EligibilityDecision)
# are defined in their home domains and never imported here.
from app.hoc.cus.hoc_spine.schemas.protocols import (
    EligibilityVerdictPort as EligibilityVerdict,
    ValidatorVerdictPort as ValidatorVerdict,
)


class _EligibilityDecisionValue:
    """Enum-like value wrapper for string comparison."""
    def __init__(self, val: str):
        self.value = val
    def __eq__(self, other):
        if isinstance(other, str):
            return self.value == other
        if isinstance(other, _EligibilityDecisionValue):
            return self.value == other.value
        return NotImplemented


class _EligibilityDecision:
    """Local proxy for EligibilityDecision enum values used in contract logic."""
    MAY_NOT = _EligibilityDecisionValue("MAY_NOT")
    MAY = _EligibilityDecisionValue("MAY")


EligibilityDecision = _EligibilityDecision()
