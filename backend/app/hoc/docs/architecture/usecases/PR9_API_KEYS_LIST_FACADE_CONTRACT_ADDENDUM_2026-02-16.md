# PR9_API_KEYS_LIST_FACADE_CONTRACT_ADDENDUM_2026-02-16

## Status
- Date: 2026-02-16
- Scope: PR-9 contract lock for API Keys List Facade implementation
...
3. Domain literature for api_keys
4. Runtime wiring + code reality
5. This PR addendum (frozen PR-9 contract)
...
- Facade endpoint:
  ...
  - Gateway exposure: `GET /hoc/api/cus/api_keys/list`
- Dispatch mapping (exactly one registry call per request):
  - `registry.execute("api_keys.query", method="list_api_keys", status, limit, offset)`
- Facade does not proxy HTTP-to-HTTP and does not call L6 drivers directly.

## 1. Scope
- PR-9 is read-only.
- Endpoint provides API keys list summary only.
- Mutations are out of scope.

## 2. Route Contract
- Backend canonical route: `GET /cus/api_keys/list`.
- Gateway path: `GET /hoc/api/cus/api_keys/list`.
- Exactly one backend route resolves; no duplicate/double-prefix alias.

## 3. Query Contract
Allowed params (strict allowlist):
- `status` (optional enum: `active | revoked | expired`)
...
- Exactly one `api_keys.query` registry call per request.
...
- In PR-9 this timestamp is generated at facade boundary.
...
| Area | Test | Expected |
|---|---|---|
| Route | Route registered once | `/cus/api_keys/list` exists once; no duplicate alias |
...
| Dispatch | List request | one `api_keys.query` call with `method=list_api_keys` |
| Pagination | Page math | `has_more` and `next_offset` derived from `total` and returned count |
| Determinism | Same request repeated | stable key ordering retained |
