...
- POST /fdr/onboarding/force-complete
...
router = APIRouter(prefix="/fdr/onboarding", tags=["Founder Onboarding"])
...
async def force_complete_onboarding(
    ...
    # Get current state (Phase A2: Onboarding SSOT)
    ...
    return ForceCompleteResponse(
        ...
        to_state=result.get("to_state", "COMPLETE"),
        reason=reason,
        actor_email=token.email,
        timestamp=datetime.now(timezone.utc).isoformat(),
        audit_id=audit_id,
