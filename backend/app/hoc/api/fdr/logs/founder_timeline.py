...
# L4 session + sql_text helpers (L2 must not import sqlalchemy directly)
...
def fetch_run_data(run_id: str, session) -> Optional[Dict[str, Any]]:
    """
...
def fetch_decision_records(run_id: str, session) -> List[Dict[str, Any]]:
    """
...
async def get_run_timeline(
    ...
    run_data = fetch_run_data(run_id, session)
    ...
    decision_records = fetch_decision_records(run_id, session)
...
async def count_decision_records(
    ...
    try:
        ...
        result = session.execute(
            sql_text(f"SELECT COUNT(*) FROM contracts.decision_records WHERE {where_sql}"),
            params,
        )
