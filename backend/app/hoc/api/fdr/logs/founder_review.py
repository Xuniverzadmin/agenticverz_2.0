...
async def list_auto_execute_decisions(
    ...
    try:
        count_result = session.execute(sql_text(count_query), params)
    ...
    try:
        result = session.execute(sql_text(paginated_query), params)
...
async def get_auto_execute_stats(
    ...
    try:
        result = session.execute(sql_text(count_query), params)
    ...
    try:
        result = session.execute(sql_text(confidence_query), params)
    ...
    flag_counts = {}
    try:
        result = session.execute(sql_text(flag_query), params)
        for row in result.fetchall():
    ...
    daily_counts = []
    try:
        result = session.execute(sql_text(daily_query), params)
        for row in result.fetchall():
            daily_counts.append(
                {
...
async def get_auto_execute_decision(
    ...
    try:
        result = session.execute(sql_text(query), {"invocation_id": invocation_id})
        row = result.fetchone()
    except Exception as e:
