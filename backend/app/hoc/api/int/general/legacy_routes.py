...
#   Trigger: api
#   Execution: sync
# Role: Legacy route handlers (410 Gone for deprecated paths)
# Callers: External clients
# Allowed Imports: L3, L4
# Forbidden Imports: L1, L5, L6
# Reference: PIN-153, M29 Category 7
...
# =============================================================================
# /api/v1/* - Legacy API Version Prefix (2026-02-06)
# =============================================================================


@router.api_route("/api/v1/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
async def legacy_api_v1(path: str = ""):
    """410 Gone - /api/v1/* prefix removed.

    The canonical HOC API no longer uses /api/v1 version prefix.
    """
    full_path = f"/api/v1/{path}" if path else "/api/v1"
    replacement = f"/{path}" if path else "/"
    return gone_response(
        path=full_path,
        reason="API version prefix removed; use canonical HOC paths without /api/v1",
