/**
 * AutoExecuteReviewContent - PIN-333
 *
 * Founder AUTO_EXECUTE Review Dashboard Content (Evidence-Only)
 *
...
function DailyTrendChart({ stats }: { stats: AutoExecuteReviewStats | undefined }) {
  ...
  const maxValue = Math.max(
    ...stats.daily_counts.map((d) => Math.max(d.executed, d.skipped, d.flagged || 0))
  );
    ...
    <div className="bg-gray-800 rounded-lg p-4 border border-gray-700 mb-6">
      ...
      <div className="flex items-end gap-1 h-32">
        {stats.daily_counts.slice(-14).map((day, idx) => {
          ...
          const flaggedHeight = maxValue > 0 ? ((day.flagged || 0) / maxValue) * 100 : 0;

          return (
            <div
              key={idx}
              className="flex-1 flex flex-col items-center gap-0.5 group relative"
              title={`${day.date}: ${day.executed} executed, ${day.skipped} skipped`}
            >
              <div className="w-full flex flex-col-reverse gap-0.5" style={{ height: '100px' }}>
                ...
                <div
                  ...
                  style={{ height: `${flaggedHeight}%`, minHeight: (day.flagged || 0) > 0 ? '2px' : '0' }}
                />
              </div>
              {idx % 2 === 0 && (
        ...
        <div className="bg-gray-900/50 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-mono text-gray-300 text-xs">{item.invocation_id}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Envelope ID</span>
              <span className="font-mono text-gray-300 text-xs">{item.envelope_id}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Tenant ID</span>
              <span className="font-mono text-gray-300 text-xs">{item.tenant_id}</span>
            </div>
          </div>
        </div>
...
export default function AutoExecuteReviewContent() {
  ...
  return (
    <>
      {/* Advisory Notice */}
      <div className="mb-6 p-3 bg-amber-900/20 border border-amber-800 rounded-lg text-sm text-amber-400">
