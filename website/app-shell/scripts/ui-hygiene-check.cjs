...
const FIX_HINTS = {
  SCOPE_CREEP: `
  Fix: app-shell/src/pages/ is bounded to LoginPage and CreditsPage only.
    1. Move new pages to website/fops/ (founder) or website/onboarding/
    2. Or amend APP_SHELL_SCOPE.md with justification and create PIN
    Reference: APP_SHELL_SCOPE.md`,
  STALE_BUTTON: `
...
function checkPageBudget() {
  ...
  for (const pageFile of pageFiles) {
    ...
    // Skip non-page files
    if (!fileName.includes('Page')) continue;

    // Check if this page is in the allowed list
    if (!ALLOWED_APP_SHELL_PAGES.includes(fileName)) {
      issues.push({
        file: path.relative(process.cwd(), pageFile),
        line: 1,
        type: 'warning',
        category: 'SCOPE_CREEP',
        message: `Page "${fileName}" violates app-shell scope contract - only LoginPage and CreditsPage allowed`,
      });
    }
  }
}
...
function main() {
  ...
  checkPageBudget();  // R1-2: Scope creep prevention
