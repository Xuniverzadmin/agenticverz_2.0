...
def generate_batch(
    known: int = 600,
    unknown: int = 300,
    recovery_success: int = 50,
    recovery_fail: int = 50,
            ...
            if failure:
                if succeeded:
                    failure.mark_recovery_succeeded(
                        by="synthetic", notes="Synthetic test"
                    )
                else:
                ...
                update_recovery_status(
                    ...
                    error_code=failure.error_code[:50]
...
def validate_db(expected_min: int = 1000) -> Dict[str, Any]:
    ...
    try:
        ...
        with Session(engine) as session:
            total = session.exec(select(func.count(FailureMatch.id))).one()
...
def validate_metrics() -> Dict[str, Any]:
    ...
    try:
        with urllib.request.urlopen(
            "http://localhost:9090/api/v1/query?query=failure_match_hits_total"
        ) as response:
            hits_data = json.loads(response.read().decode())

        with urllib.request.urlopen(
            "http://localhost:9090/api/v1/query?query=failure_match_misses_total"
        ) as response:
            misses_data = json.loads(response.read().decode())

        with urllib.request.urlopen(
            "http://localhost:9090/api/v1/query?query=recovery_success_total"
        ) as response:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic failure traffic for M9 validation"
    )
    parser.add_argument(
        "--known", type=int, default=600, help="Number of known failures"
    )
    parser.add_argument(
        "--unknown", type=int, default=300, help="Number of unknown failures"
    )
    parser.add_argument(
        "--recovery-success", type=int, default=50, help="Recovery success attempts"
    )
    parser.add_argument(
        "--recovery-fail", type=int, default=50, help="Recovery failure attempts"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Generate but don't persist"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate after generation"
    )
    parser.add_argument(
    ...
    if args.dry_run:
        ...
        print(
            f"  Recovery attempts: {sum(1 for f in failures if f.is_recovery_attempt)}"
        )
    ...
    if args.validate:
        ...
        print("\nMetrics validation:")
        print(
            f"  Prometheus reachable: {metrics_result.get('prometheus_reachable', False)}"
        )
