...
__export(index_exports, {
  ...
  createTraceFromContext: () => createTraceFromContext,
  diffTraces: () => diffTraces,
  freezeTime: () => freezeTime,
  generateIdempotencyKey: () => generateIdempotencyKey,
  hashData: () => hashData,
  hashTrace: () => hashTrace,
  isIdempotencyKeyExecuted: () => isIdempotencyKeyExecuted,
  markIdempotencyKeyExecuted: () => markIdempotencyKeyExecuted,
  replayStep: () => replayStep,
  resetIdempotencyState: () => resetIdempotencyState
});
...
var RuntimeContext = class _RuntimeContext {
  ...
  _captureRngState() {
    ...
    return (0, import_crypto.createHash)("sha256").update(stateStr).digest("hex").slice(0, 16);
  }
  /**
...
function hashData(data) {
  ...
  return (0, import_crypto.createHash)("sha256").update(canonical).digest("hex").slice(0, 16);
}
...
var Trace = class _Trace {
  ...
  _computeRootHash() {
    ...
    let chainHash = (0, import_crypto2.createHash)("sha256").update(base).digest("hex");
    for (const step of this.steps) {
      const stepDetHash = step.deterministicHash();
      const combined = `${chainHash}:${stepDetHash}`;
      chainHash = (0, import_crypto2.createHash)("sha256").update(combined).digest("hex");
    }
    return chainHash;
  }
  /**
   ...
   */
  save(path) {
    (0, import_fs.writeFileSync)(path, this.toJson());
  }
  /**
   * Load trace from file.
   */
  static load(path) {
    return _Trace.fromJson((0, import_fs.readFileSync)(path, "utf-8"));
  }
};
...
function generateIdempotencyKey(runId, stepIndex, skillId, inputHash) {
  ...
  return (0, import_crypto2.createHash)("sha256").update(keyData).digest("hex").slice(0, 32);
}
