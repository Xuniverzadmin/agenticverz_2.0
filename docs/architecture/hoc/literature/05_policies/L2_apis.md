...
**Layer:** L2_api | **Domain:** policies | **Lines:** 1586
...
| `auth.console_auth` | FounderToken, verify_fops_token | yes |
...
| `app.hoc.cus.general.L5_engines.alerts_facade` | AlertsFacade, get_alerts_facade | no |
...
|--------|-------------|-------------|------|
| `from app.hoc.cus.general.L5_engines.alerts_facade import AlertsFacade, get_alerts_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 46 |

---

## analytics.py
**Path:** `backend/app/hoc/api/cus/policies/analytics.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 1067
...
| `UsageStatisticsResponse` |  | GET /api/v1/analytics/statistics/usage response (contracted). |
...
| `CostStatisticsResponse` |  | GET /api/v1/analytics/statistics/cost response (contracted). |
| `TopicStatus` |  | Status of a topic within a subdomain. |
| `AnalyticsStatusResponse` |  | GET /api/v1/analytics/_status response. |
...
| `app.hoc.cus.analytics.L5_engines.analytics_facade` | get_analytics_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.analytics.L5_engines.analytics_facade import get_analytics_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 57 |

---

## aos_accounts.py
**Path:** `backend/app/hoc/api/cus/policies/aos_accounts.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 1277
...
| `get_project_detail` | `(request: Request, project_id: str, session: AsyncSession = Depends(get_async_se` | yes | READ-ONLY customer facade - delegates to L4 AccountsFacade. |
...
| `get_user_detail` | `(request: Request, user_id: str, session: AsyncSession = Depends(get_async_sessi` | yes | READ-ONLY customer facade - delegates to L4 AccountsFacade. |
| `get_profile` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Pr` | yes | READ-ONLY customer facade - delegates to L4 AccountsFacade. |
| `get_billing_summary` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Bi` | yes | READ-ONLY customer facade - delegates to L4 AccountsFacade. |
| `update_profile` | `(request: Request, update: ProfileUpdateRequest, session: AsyncSession = Depends` | yes | WRITE customer facade - delegates to L4 AccountsFacade. |
| `get_billing_invoices` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> In` | yes | READ-ONLY customer facade - delegates to L4 AccountsFacade. |
| `get_support_contact` | `() -> SupportContactResponse` | yes | READ-ONLY customer facade - delegates to L4 AccountsFacade. |
| `create_support_ticket` | `(request: Request, ticket: SupportTicketCreate, session: AsyncSession = Depends(` | yes | WRITE customer facade - delegates to L4 AccountsFacade. |
...
| `invite_user` | `(request: Request, invite: InviteUserRequest, session: AsyncSession = Depends(ge` | yes | WRITE customer facade - delegates to L4 AccountsFacade. |
...
| `accept_invitation` | `(invitation_id: str, accept: AcceptInvitationRequest, session: AsyncSession = De` | yes | Accept an invitation to join a tenant. |
| `list_tenant_users` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Te` | yes | List users in the current tenant. |
| `update_user_role` | `(request: Request, user_id: str, update: UpdateUserRoleRequest, session: AsyncSe` | yes | Update a user's role in the tenant. |
| `remove_user` | `(request: Request, user_id: str, session: AsyncSession = Depends(get_async_sessi` | yes | Remove a user from the tenant. |
...
| `app.auth.gateway_middleware` | get_auth_context | no |
| `app.db` | get_async_session_dep | no |
| `app.hoc.cus.account.L5_engines.accounts_facade` | get_accounts_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.account.L5_engines.accounts_facade import get_accounts_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 65 |
| `from app.models.tenant import Invitation, Subscription, SupportTicket, Tenant, TenantMembership, User, generate_uuid, utc_now` | L2 MUST NOT import L7 models | Use L5 schemas or response models | 66 |

---

## aos_api_key.py
**Path:** `backend/app/hoc/api/cus/policies/aos_api_key.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 272
...
|------|-----------|-------|-----------|
| `get_tenant_id_from_auth` | `(request: Request) -> str` | no | Extract tenant_id from auth_context. Raises 401/403 if missing. |
| `list_api_keys` | `(request: Request, status: Annotated[Optional[str], Query(description='Filter by` | yes | List API keys. READ-ONLY. Delegates to L4 APIKeysFacade. |
| `get_api_key_detail` | `(request: Request, key_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | Get API key detail (O3). Delegates to L4 APIKeysFacade. |
...
| `app.auth.gateway_middleware` | get_auth_context | no |
| `app.db` | get_async_session_dep | no |
| `app.hoc.cus.api_keys.L5_engines.api_keys_facade` | get_api_keys_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.api_keys.L5_engines.api_keys_facade import get_api_keys_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 49 |

---

## aos_cus_integrations.py
**Path:** `backend/app/hoc/api/cus/policies/aos_cus_integrations.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 570
...
| `app.hoc.cus.integrations.L5_engines.integrations_facade` | get_integrations_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.integrations.L5_engines.integrations_facade import get_integrations_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 69 |

---

## billing_dependencies.py
**Path:** `backend/app/hoc/api/cus/policies/billing_dependencies.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 192
...
| `app.billing.provider` | get_billing_provider | no |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.general.L5_engines.compliance_facade` | ComplianceFacade, get_compliance_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.general.L5_engines.compliance_facade import ComplianceFacade, get_compliance_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 40 |

---

## connectors.py
**Path:** `backend/app/hoc/api/cus/policies/connectors.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 235
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> ConnectorsFacade` | no | Get the connectors facade. |
...
| `get_connector` | `(connector_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Co` | yes | Get a specific connector by ID. |
| `update_connector` | `(connector_id: str, request: UpdateConnectorRequest, ctx: TenantContext = Depend` | yes | Update a connector. |
| `delete_connector` | `(connector_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Co` | yes | Delete a connector. |
| `test_connector` | `(connector_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Co` | yes | Test a connector connection. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.integrations.L5_engines.connectors_facade` | ConnectorsFacade, get_connectors_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.integrations.L5_engines.connectors_facade import ConnectorsFacade, get_connectors_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 40 |

---

## controls.py
**Path:** `backend/app/hoc/api/cus/policies/controls.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 219
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> ControlsFacade` | no | Get the controls facade. |
| `list_controls` | `(control_type: Optional[str] = Query(None, description='Filter by type'), state:` | yes | List controls (GAP-123). |
| `get_status` | `(ctx: TenantContext = Depends(get_tenant_context), facade: ControlsFacade = Depe` | yes | Get overall control status. |
| `get_control` | `(control_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Cont` | yes | Get a specific control. |
| `update_control` | `(control_id: str, request: UpdateControlRequest, ctx: TenantContext = Depends(ge` | yes | Update a control. |
| `enable_control` | `(control_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Cont` | yes | Enable a control. |
| `disable_control` | `(control_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Cont` | yes | Disable a control. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.controls.L5_engines.controls_facade` | ControlsFacade, get_controls_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.controls.L5_engines.controls_facade import ControlsFacade, get_controls_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 42 |

---

## cus_enforcement.py
**Path:** `backend/app/hoc/api/cus/policies/cus_enforcement.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 222
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_enforcement_service` | `() -> CusEnforcementService` | no | Dependency to get enforcement service instance. |
| `get_tenant_id` | `(request: Request) -> str` | no | Extract tenant_id from authenticated request. |
| `check_enforcement` | `(payload: EnforcementCheckRequest, request: Request, service: CusEnforcementServ` | yes | Check enforcement policy before making an LLM call. |
...
| `batch_enforcement_check` | `(payload: EnforcementBatchRequest, request: Request, service: CusEnforcementServ` | yes | Check enforcement for multiple requests at once. |
...
| `pydantic` | BaseModel, Field | no |
| `app.schemas.response` | wrap_dict, wrap_list | no |
| `app.hoc.cus.policies.L5_engines.cus_enforcement_service` | CusEnforcementService | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.cus_enforcement_service import CusEnforcementService` | L2 MUST NOT import L5 directly | Route through L3 adapter | 41 |

---

## customer_visibility.py
**Path:** `backend/app/hoc/api/cus/policies/customer_visibility.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 707
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_db_url` | `() -> Optional[str]` | no | Get database URL from environment. |
...
| `fetch_run_outcome` | `(run_id: str, tenant_id: str) -> Optional[Dict[str, Any]]` | no | Fetch run data for outcome reconciliation. |
| `fetch_decision_summary` | `(run_id: str, tenant_id: str) -> Dict[str, Any]` | no | Fetch decision summary for outcome reconciliation. |
...
| `get_outcome_reconciliation` | `(run_id: str, request: Request, _: str = Depends(verify_api_key)) -> OutcomeReco` | yes | Get outcome reconciliation after execution. |
...
**Layer:** L2_api | **Domain:** policies | **Lines:** 305
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> DataSourcesFacade` | no | Get the datasources facade. |
...
| `get_statistics` | `(ctx: TenantContext = Depends(get_tenant_context), facade: DataSourcesFacade = D` | yes | Get data source statistics. |
| `get_source` | `(source_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: DataS` | yes | Get a specific data source. |
| `update_source` | `(source_id: str, request: UpdateSourceRequest, ctx: TenantContext = Depends(get_` | yes | Update a data source. |
| `delete_source` | `(source_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: DataS` | yes | Delete a data source. |
| `test_connection` | `(source_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: DataS` | yes | Test a data source connection. |
| `activate_source` | `(source_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: DataS` | yes | Activate a data source. |
| `deactivate_source` | `(source_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: DataS` | yes | Deactivate a data source. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.integrations.L5_engines.datasources_facade` | DataSourcesFacade, get_datasources_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.integrations.L5_engines.datasources_facade import DataSourcesFacade, get_datasources_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 43 |

---

## detection.py
**Path:** `backend/app/hoc/api/cus/policies/detection.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 244
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> DetectionFacade` | no | Get the detection facade. |
...
| `get_anomaly` | `(anomaly_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Dete` | yes | Get a specific anomaly by ID. |
| `resolve_anomaly` | `(anomaly_id: str, request: ResolveAnomalyRequest, ctx: TenantContext = Depends(g` | yes | Resolve an anomaly. |
| `acknowledge_anomaly` | `(anomaly_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Dete` | yes | Acknowledge an anomaly. |
| `get_detection_status` | `(ctx: TenantContext = Depends(get_tenant_context), facade: DetectionFacade = Dep` | yes | Get detection engine status. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.analytics.L5_engines.detection_facade` | DetectionFacade, get_detection_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.analytics.L5_engines.detection_facade import DetectionFacade, get_detection_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 40 |

---

## evidence.py
**Path:** `backend/app/hoc/api/cus/policies/evidence.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 299
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> EvidenceFacade` | no | Get the evidence facade. |
...
| `get_chain` | `(chain_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Eviden` | yes | Get a specific evidence chain. |
| `add_evidence` | `(chain_id: str, request: AddEvidenceRequest, ctx: TenantContext = Depends(get_te` | yes | Add evidence to a chain. |
| `verify_chain` | `(chain_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Eviden` | yes | Verify chain integrity. |
...
| `get_export` | `(export_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Evide` | yes | Get export status. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.logs.L5_engines.evidence_facade` | EvidenceFacade, get_evidence_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.logs.L5_engines.evidence_facade import EvidenceFacade, get_evidence_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 42 |

---

## governance.py
**Path:** `backend/app/hoc/api/cus/policies/governance.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 290
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> GovernanceFacade` | no | Get the governance facade. |
| `get_governance_state` | `(ctx: TenantContext = Depends(get_tenant_context), facade: GovernanceFacade = De` | yes | Get current governance state. |
| `toggle_kill_switch` | `(request: KillSwitchRequest, ctx: TenantContext = Depends(get_tenant_context), f` | yes | Toggle the governance kill switch (GAP-090). |
| `set_governance_mode` | `(request: ModeRequest, ctx: TenantContext = Depends(get_tenant_context), facade:` | yes | Set governance mode (GAP-091). |
...
| `get_boot_status` | `(ctx: TenantContext = Depends(get_tenant_context), facade: GovernanceFacade = De` | yes | Get SPINE component health status (GAP-095). |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.policies.L5_engines.governance_facade` | GovernanceFacade, GovernanceMode, get_governance_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.governance_facade import GovernanceFacade, GovernanceMode, get_governance_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 40 |

---

## guard.py
**Path:** `backend/app/hoc/api/cus/policies/guard.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 2220
...
|------|-----------|-------|-----------|
| `utc_now` | `() -> datetime` | no |  |
| `get_tenant_from_auth` | `(session: Session, tenant_id: str) -> Tenant` | no | Get tenant or raise 404. |
| `get_guard_status` | `(tenant_id: str = Query(..., description='Tenant ID'), session: Session = Depend` | yes | Get protection status - "Am I safe right now?" |
| `get_today_snapshot` | `(tenant_id: str = Query(..., description='Tenant ID'), session: Session = Depend` | yes | Get today's metrics - "What did it cost/save me?" |
| `activate_killswitch` | `(tenant_id: str = Query(..., description='Tenant ID'), session: Session = Depend` | yes | Stop all traffic - Emergency kill switch. |
| `deactivate_killswitch` | `(tenant_id: str = Query(..., description='Tenant ID'), session: Session = Depend` | yes | Resume traffic - Deactivate kill switch. |
...
| `_generate_plain_title` | `(incident: Incident) -> str` | no | Generate plain language title - no internal terminology. |
| `_generate_calm_summary` | `(incident: Incident) -> str` | no | Generate calm, reassuring summary - no internal terms. |
| `_build_customer_impact` | `(incident: Incident) -> CustomerIncidentImpactDTO` | no | Build impact assessment with calm vocabulary. |
| `_build_customer_resolution` | `(incident: Incident) -> CustomerIncidentResolutionDTO` | no | Build resolution status with reassuring message. |
| `_build_customer_actions` | `(incident: Incident) -> list` | no | Build customer actions - only if necessary. |
| `replay_call` | `(call_id: str, level: str = Query('logical', description='Determinism level: str` | yes | Replay a call - Trust builder. |
| `list_api_keys` | `(tenant_id: str = Query(..., description='Tenant ID'), session: Session = Depend` | yes | List API keys with status. |
| `freeze_api_key` | `(key_id: str, tenant_id: str = Query(..., description='Tenant ID'), session: Ses` | yes | Freeze an API key. |
| `unfreeze_api_key` | `(key_id: str, tenant_id: str = Query(..., description='Tenant ID'), session: Ses` | yes | Unfreeze an API key. |
| `get_settings` | `(tenant_id: str = Query(..., description='Tenant ID'), session: Session = Depend` | yes | Get read-only settings. |
...
| `get_decision_timeline` | `(incident_id: str, session: Session = Depends(get_session))` | yes | Get decision timeline - M23 component map spec. |
...
| `app.models.killswitch` | DefaultGuardrail, Incident, IncidentEvent, IncidentSeverity, KillSwitchState (+1) | no |
| `app.models.tenant` | Tenant | no |
| `app.hoc.cus.logs.L5_engines.certificate` | CertificateService | no |
| `app.hoc.cus.general.L5_controls.drivers.guard_write_driver` | GuardWriteDriver | no |
| `app.hoc.cus.logs.L5_engines.replay_determinism` | DeterminismLevel, ReplayContextBuilder | no |
| `app.hoc.cus.logs.L5_engines.replay_determinism` | ReplayValidator | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.models.killswitch import DefaultGuardrail, Incident, IncidentEvent, IncidentSeverity, KillSwitchState, ProxyCall` | L2 MUST NOT import L7 models | Use L5 schemas or response models | 71 |
| `from app.models.tenant import Tenant` | L2 MUST NOT import L7 models | Use L5 schemas or response models | 79 |
| `from app.hoc.cus.logs.L5_engines.certificate import CertificateService` | L2 MUST NOT import L5 directly | Route through L3 adapter | 83 |
| `from app.hoc.cus.logs.L5_engines.replay_determinism import DeterminismLevel, ReplayContextBuilder` | L2 MUST NOT import L5 directly | Route through L3 adapter | 93 |
| `from app.hoc.cus.logs.L5_engines.replay_determinism import ReplayValidator` | L2 MUST NOT import L5 directly | Route through L3 adapter | 97 |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.general.L5_engines.lifecycle_facade` | LifecycleFacade, get_lifecycle_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.general.L5_engines.lifecycle_facade import LifecycleFacade, get_lifecycle_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 49 |

---

## logs.py
**Path:** `backend/app/hoc/api/cus/policies/logs.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 1433
...
| `get_llm_run_envelope` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O1: Canonical immutable run record. READ-ONLY customer facade. |
| `get_llm_run_trace` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O2: Step-by-step execution trace. READ-ONLY customer facade. |
| `get_llm_run_governance` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O3: Policy interaction trace. READ-ONLY customer facade. |
| `get_llm_run_replay` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O4: 60-second replay window. READ-ONLY customer facade. |
| `get_llm_run_export` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O5: Export information. READ-ONLY customer facade. |
...
| `get_system_snapshot` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O1: Environment baseline snapshot. READ-ONLY customer facade. |
...
| `get_system_events` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O3: Infra events affecting run. READ-ONLY customer facade. |
| `get_system_replay` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | O4: Infra replay window. READ-ONLY customer facade. |
...
| `get_audit_entry` | `(request: Request, entry_id: str, session: AsyncSession = Depends(get_async_sess` | yes | Get audit entry detail. READ-ONLY customer facade. |
| `get_audit_identity` | `(request: Request, limit: Annotated[int, Query(ge=1, le=100)] = 50, session: Asy` | yes | O1: Identity lifecycle. READ-ONLY customer facade. |
| `get_audit_authorization` | `(request: Request, limit: Annotated[int, Query(ge=1, le=100)] = 50, session: Asy` | yes | O2: Authorization decisions. READ-ONLY customer facade. |
| `get_audit_access` | `(request: Request, limit: Annotated[int, Query(ge=1, le=100)] = 50, session: Asy` | yes | O3: Log access audit. READ-ONLY customer facade. |
...
| `app.auth.gateway_middleware` | get_auth_context | no |
| `app.db` | get_async_session_dep | no |
| `app.hoc.cus.logs.L5_engines.logs_facade` | get_logs_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.logs.L5_engines.logs_facade import get_logs_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 71 |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.general.L5_engines.monitors_facade` | MonitorsFacade, get_monitors_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.general.L5_engines.monitors_facade import MonitorsFacade, get_monitors_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 44 |
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> NotificationsFacade` | no | Get the notifications facade. |
...
| `list_channels` | `(ctx: TenantContext = Depends(get_tenant_context), facade: NotificationsFacade =` | yes | List available notification channels. |
| `get_preferences` | `(ctx: TenantContext = Depends(get_tenant_context), facade: NotificationsFacade =` | yes | Get notification preferences for the current user. |
...
| `get_notification` | `(notification_id: str, ctx: TenantContext = Depends(get_tenant_context), facade:` | yes | Get a specific notification. |
| `mark_as_read` | `(notification_id: str, ctx: TenantContext = Depends(get_tenant_context), facade:` | yes | Mark a notification as read. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.account.L5_engines.notifications_facade` | NotificationsFacade, get_notifications_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.account.L5_engines.notifications_facade import NotificationsFacade, get_notifications_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 40 |

---

## override.py
**Path:** `backend/app/hoc/api/cus/policies/override.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 335
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `create_override` | `(request: Request, body: CreateOverrideRequest, session: AsyncSession = Depends(` | yes | Request a temporary limit override. |
| `list_overrides` | `(request: Request, status: Optional[str] = Query(default=None, description='Filt` | yes | List overrides for the tenant. |
| `get_override` | `(request: Request, override_id: str, session: AsyncSession = Depends(get_async_s` | yes | Get override by ID. |
| `cancel_override` | `(request: Request, override_id: str, session: AsyncSession = Depends(get_async_s` | yes | Cancel a pending or active override. |
...
| `app.hoc.cus.controls.L6_drivers.override_driver` | LimitNotFoundError, LimitOverrideService, LimitOverrideServiceError, OverrideNotFoundError, OverrideValidationError (+1) | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.controls.L6_drivers.override_driver import LimitNotFoundError, LimitOverrideService, LimitOverrideServiceError, OverrideNotFoundError, OverrideValidationError, StackingAbuseError` | L2 MUST NOT import L6 | Route through L3 → L5 → L6 | 43 |

---

## policies.py
**Path:** `backend/app/hoc/api/cus/policies/policies.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 1531
...
| `get_policy_rule_detail` | `(request: Request, rule_id: str, session: AsyncSession = Depends(get_async_sessi` | yes | Get policy rule detail (O3). Tenant isolation enforced. |
...
| `get_limit_detail` | `(request: Request, limit_id: str, session: AsyncSession = Depends(get_async_sess` | yes | Get limit detail (O3). Tenant isolation enforced. |
...
| `get_lesson_stats` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Le` | yes | Get lesson statistics (O1). READ-ONLY customer facade. |
| `get_lesson_detail` | `(request: Request, lesson_id: str, session: AsyncSession = Depends(get_async_ses` | yes | Get lesson detail (O3). READ-ONLY customer facade. |
| `get_policy_state` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Po` | yes | Get policy layer state (ACT-O4). Customer facade. |
...
| `app.db` | get_async_session_dep | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.policies.L5_engines.policies_facade` | get_policies_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.policies_facade import get_policies_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 52 |
...
**Layer:** L2_api | **Domain:** policies | **Lines:** 2042
...
| `_get_approval_level_config` | `(session: AsyncSession, policy_type: PolicyType, tenant_id: str, agent_id: Optio` | yes | Get approval level configuration from PolicyApprovalLevel table. |
| `_config_to_dict` | `(config) -> Dict[str, Any]` | no | Convert PolicyApprovalLevel to dict. |
...
| `evaluate_policy` | `(request: PolicyEvalRequest, session: AsyncSession = Depends(get_async_session),` | yes | Sandbox evaluation of policy for a skill execution. |
| `create_approval_request` | `(request: ApprovalRequestCreate, background_tasks: BackgroundTasks, session: Asy` | yes | Create a new approval request (persisted to DB). |
| `get_approval_request` | `(request_id: str, session: AsyncSession = Depends(get_async_session)) -> Approva` | yes | Get the current status of an approval request. |
...
| `run_escalation_check` | `(session: AsyncSession) -> int` | yes | Check for pending requests that need escalation. |
...
| `get_active_policy_detail` | `(policy_id: str, session: AsyncSession = Depends(get_async_session), ctx: Tenant` | yes | V2 Facade: Policy detail for cross-domain navigation. |
...
| `get_policy_threshold_detail` | `(threshold_id: str, session: AsyncSession = Depends(get_async_session), ctx: Ten` | yes | V2 Facade: Threshold detail for cross-domain navigation. |
...
| `get_policy_violation_detail` | `(violation_id: str, session: AsyncSession = Depends(get_async_session), ctx: Ten` | yes | V2 Facade: Violation detail for cross-domain navigation. |
...
| `app.db_async` | get_async_session | no |
| `app.schemas.response` | wrap_dict | no |
...
**Layer:** L2_api | **Domain:** policies | **Lines:** 1465
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `evaluate_action` | `(request: EvaluateRequest, db: AsyncSession = Depends(get_async_session)) -> Pol` | yes | Evaluate a proposed action against all applicable policies. |
| `simulate_evaluation` | `(request: SimulateRequest, db: AsyncSession = Depends(get_async_session)) -> Pol` | yes | Simulate policy evaluation without side effects. |
| `get_policy_state` | `(db: AsyncSession = Depends(get_async_session)) -> PolicyState` | yes | Get the current state of the policy layer. |
| `reload_policies` | `(db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]` | yes | Hot-reload policies from database. |
...
| `get_violation` | `(violation_id: str, db: AsyncSession = Depends(get_async_session)) -> PolicyViol` | yes | Get a specific violation by ID. |
| `acknowledge_violation` | `(violation_id: str, notes: Optional[str] = None, db: AsyncSession = Depends(get_` | yes | Acknowledge a violation (mark as reviewed). |
| `list_risk_ceilings` | `(tenant_id: Optional[str] = None, include_inactive: bool = False, db: AsyncSessi` | yes | List all risk ceilings with current values. |
| `get_risk_ceiling` | `(ceiling_id: str, db: AsyncSession = Depends(get_async_session)) -> Dict[str, An` | yes | Get a specific risk ceiling with current utilization. |
| `update_risk_ceiling` | `(ceiling_id: str, update: RiskCeilingUpdate, db: AsyncSession = Depends(get_asyn` | yes | Update a risk ceiling configuration. |
| `reset_risk_ceiling` | `(ceiling_id: str, db: AsyncSession = Depends(get_async_session)) -> Dict[str, An` | yes | Reset a risk ceiling's current value to 0. |
| `list_safety_rules` | `(tenant_id: Optional[str] = None, include_inactive: bool = False, db: AsyncSessi` | yes | List all safety rules. |
| `update_safety_rule` | `(rule_id: str, update: SafetyRuleUpdate, db: AsyncSession = Depends(get_async_se` | yes | Update a safety rule configuration. |
| `list_ethical_constraints` | `(include_inactive: bool = False, db: AsyncSession = Depends(get_async_session)) ` | yes | List all ethical constraints. |
| `list_active_cooldowns` | `(agent_id: Optional[str] = None, db: AsyncSession = Depends(get_async_session)) ` | yes | List all active cooldowns. |
| `clear_cooldowns` | `(agent_id: str, rule_name: Optional[str] = None, db: AsyncSession = Depends(get_` | yes | Clear cooldowns for an agent. |
| `get_policy_metrics` | `(hours: int = Query(24, ge=1, le=720), db: AsyncSession = Depends(get_async_sess` | yes | Get policy engine metrics for the specified time window. |
| `evaluate_batch` | `(requests: List[EvaluateRequest], db: AsyncSession = Depends(get_async_session))` | yes | Evaluate multiple actions in a single call. |
| `list_policy_versions` | `(limit: int = Query(20, ge=1, le=100), include_inactive: bool = False, db: Async` | yes | List all policy versions. |
| `get_current_version` | `(db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]` | yes | Get the currently active policy version. |
| `create_policy_version` | `(request: CreateVersionRequest, db: AsyncSession = Depends(get_async_session)) -` | yes | Create a new policy version snapshot. |
| `rollback_to_version` | `(request: RollbackRequest, db: AsyncSession = Depends(get_async_session)) -> Dic` | yes | Rollback to a previous policy version. |
| `get_version_provenance` | `(version_id: str, db: AsyncSession = Depends(get_async_session)) -> List[Dict[st` | yes | Get the provenance (change history) for a policy version. |
| `get_dependency_graph` | `(db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]` | yes | Get the policy dependency graph. |
| `list_conflicts` | `(include_resolved: bool = False, db: AsyncSession = Depends(get_async_session)) ` | yes | List policy conflicts. |
| `resolve_conflict` | `(conflict_id: str, request: ResolveConflictRequest, db: AsyncSession = Depends(g` | yes | Resolve a policy conflict. |
| `list_temporal_policies` | `(metric: Optional[str] = None, include_inactive: bool = False, db: AsyncSession ` | yes | List temporal (sliding window) policies. |
| `create_temporal_policy` | `(request: TemporalPolicyCreate, db: AsyncSession = Depends(get_async_session)) -` | yes | Create a new temporal policy. |
| `get_temporal_utilization` | `(policy_id: str, agent_id: Optional[str] = None, db: AsyncSession = Depends(get_` | yes | Get current utilization for a temporal policy. |
| `evaluate_with_context` | `(request: ContextAwareEvaluateRequest, db: AsyncSession = Depends(get_async_sess` | yes | Context-aware policy evaluation (GAP 4). |
| `validate_dependency_dag` | `(db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]` | yes | Validate that policy dependencies form a valid DAG. |
| `add_dependency_with_dag_check` | `(request: AddDependencyRequest, db: AsyncSession = Depends(get_async_session)) -` | yes | Add a policy dependency with DAG validation. |
| `get_evaluation_order` | `(db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]` | yes | Get the topological evaluation order for policies. |
| `prune_temporal_metrics` | `(request: PruneTemporalMetricsRequest, db: AsyncSession = Depends(get_async_sess` | yes | Prune and compact temporal metric events. |
| `get_temporal_storage_stats` | `(db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any]` | yes | Get storage statistics for temporal metrics. |
| `activate_policy_version` | `(request: ActivateVersionRequest, db: AsyncSession = Depends(get_async_session))` | yes | Activate a policy version with pre-activation integrity checks. |
| `check_version_integrity` | `(version_id: str, db: AsyncSession = Depends(get_async_session)) -> Dict[str, An` | yes | Run integrity checks on a version without activating. |
| `list_lessons` | `(tenant_id: Optional[str] = None, lesson_type: Optional[str] = Query(None, descr` | yes | List lessons learned. |
| `get_lesson_stats` | `(tenant_id: str, db: AsyncSession = Depends(get_async_session)) -> Dict[str, Any` | yes | Get lesson statistics for a tenant. |
| `get_lesson` | `(lesson_id: str, tenant_id: str, db: AsyncSession = Depends(get_async_session)) ` | yes | Get a specific lesson by ID. |
...
| `app.hoc.cus.policies.L5_engines.policy_driver` | get_policy_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.policy_driver import get_policy_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 39 |

---

## policy_limits_crud.py
**Path:** `backend/app/hoc/api/cus/policies/policy_limits_crud.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 539
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `create_limit` | `(request: Request, body: CreateLimitRequest, session: AsyncSession = Depends(get` | yes | Create a new policy limit. |
| `update_limit` | `(request: Request, limit_id: str, body: UpdateLimitRequest, session: AsyncSessio` | yes | Update an existing policy limit. |
| `delete_limit` | `(request: Request, limit_id: str, session: AsyncSession = Depends(get_async_sess` | yes | Soft-delete a policy limit. |
| `get_threshold_params` | `(request: Request, limit_id: str, session: AsyncSession = Depends(get_async_sess` | yes | Get threshold parameters for a limit. |
| `set_threshold_params` | `(request: Request, limit_id: str, body: ThresholdParamsRequest, session: AsyncSe` | yes | Set threshold parameters for a limit. |
...
| `app.hoc.cus.policies.L5_engines.policy_limits_engine` | ImmutableFieldError, LimitNotFoundError, LimitValidationError, PolicyLimitsService, PolicyLimitsServiceError | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.policy_limits_engine import ImmutableFieldError, LimitNotFoundError, LimitValidationError, PolicyLimitsService, PolicyLimitsServiceError` | L2 MUST NOT import L5 directly | Route through L3 adapter | 45 |

---

## policy_proposals.py
**Path:** `backend/app/hoc/api/cus/policies/policy_proposals.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 465
...
| `auth.role_guard` | require_role | yes |
| `auth.tenant_roles` | TenantRole | yes |
| `db` | get_async_session | yes |
| `models.policy` | PolicyApprovalRequest, PolicyProposal, PolicyVersion | yes |
| `schemas.response` | wrap_dict | yes |
| `services.policy_proposal` | review_policy_proposal | yes |
...
**Layer:** L2_api | **Domain:** policies | **Lines:** 263
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `create_rule` | `(request: Request, body: CreateRuleRequest, session: AsyncSession = Depends(get_` | yes | Create a new policy rule. |
| `update_rule` | `(request: Request, rule_id: str, body: UpdateRuleRequest, session: AsyncSession ` | yes | Update an existing policy rule. |
...
| `app.hoc.cus.policies.L5_engines.policy_rules_engine` | PolicyRulesService, PolicyRulesServiceError, RuleNotFoundError, RuleValidationError | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.policy_rules_engine import PolicyRulesService, PolicyRulesServiceError, RuleNotFoundError, RuleValidationError` | L2 MUST NOT import L5 directly | Route through L3 adapter | 41 |

---

## rate_limits.py
**Path:** `backend/app/hoc/api/cus/policies/rate_limits.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 225
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `get_facade` | `() -> LimitsFacade` | no | Get the limits facade. |
| `list_limits` | `(limit_type: Optional[str] = Query(None, description='Filter by type'), limit: i` | yes | List limits (GAP-122). |
| `get_usage` | `(ctx: TenantContext = Depends(get_tenant_context), facade: LimitsFacade = Depend` | yes | Get current usage summary. |
| `check_limit` | `(request: CheckLimitRequest, ctx: TenantContext = Depends(get_tenant_context), f` | yes | Check if a limit allows an operation. |
| `get_limit` | `(limit_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Limits` | yes | Get a specific limit. |
| `update_limit` | `(limit_id: str, request: UpdateLimitRequest, ctx: TenantContext = Depends(get_te` | yes | Update a limit configuration. |
| `reset_limit` | `(limit_id: str, ctx: TenantContext = Depends(get_tenant_context), facade: Limits` | yes | Reset a limit's usage counter. |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.policies.L5_engines.limits_facade` | LimitsFacade, get_limits_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.limits_facade import LimitsFacade, get_limits_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 42 |
...
| `pydantic` | BaseModel | no |
| `sqlalchemy` | text | no |
| `auth.authorization_choke` | check_permission_request | yes |
| `auth.rbac_engine` | get_rbac_engine | yes |
| `db` | get_session | yes |
| `schemas.response` | wrap_dict | yes |
...
**Layer:** L2_api | **Domain:** policies | **Lines:** 610
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `_categorize_proxy_call` | `(call: ProxyCall) -> ReplayCategory` | no | Categorize a proxy call into replay category. |
| `_proxy_call_to_replay_item` | `(call: ProxyCall) -> ReplayItem` | no | Convert ProxyCall to ReplayItem for visualization. |
| `_incident_event_to_replay_item` | `(event: IncidentEvent) -> ReplayItem` | no | Convert IncidentEvent to ReplayItem for visualization. |
...
| `sqlmodel` | Session | no |
| `auth.authority` | AuthorityResult, require_replay_read, verify_tenant_access | yes |
| `db` | get_session | yes |
| `schemas.response` | wrap_dict | yes |
| `models.killswitch` | Incident, IncidentEvent, ProxyCall | yes |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.general.L5_engines.retrieval_facade` | RetrievalFacade, get_retrieval_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.general.L5_engines.retrieval_facade import RetrievalFacade, get_retrieval_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 42 |
...
| `app.auth.tier_gating` | requires_feature | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.general.L5_engines.scheduler_facade` | SchedulerFacade, get_scheduler_facade | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.general.L5_engines.scheduler_facade import SchedulerFacade, get_scheduler_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 44 |

---

## simulate.py
**Path:** `backend/app/hoc/api/cus/policies/simulate.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 156
...
| Name | Signature | Async | Docstring |
|------|-----------|-------|-----------|
| `simulate_execution` | `(request: Request, body: SimulateRequest, session: AsyncSession = Depends(get_as` | yes | Simulate an execution against all limits. |
...
| `app.hoc.cus.policies.L5_engines.limits_simulation_service` | LimitsSimulationService, LimitsSimulationServiceError, TenantNotFoundError | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.hoc.cus.policies.L5_engines.limits_simulation_service import LimitsSimulationService, LimitsSimulationServiceError, TenantNotFoundError` | L2 MUST NOT import L5 directly | Route through L3 adapter | 42 |

---

## status_history.py
**Path:** `backend/app/hoc/api/cus/policies/status_history.py`  
**Layer:** L2_api | **Domain:** policies | **Lines:** 466
...
| `create_export` | `(request: ExportRequest, session: Session = Depends(get_session))` | yes | Create an export of status history records. |
...
| `app.db` | StatusHistory, get_session | no |
...
**Layer:** L2_api | **Domain:** policies | **Lines:** 1607
...
| `_get_run` | `(run_id: str) -> Optional[Dict[str, Any]]` | yes | Get a run from PostgreSQL. |
| `_list_runs` | `(limit: int = 20, tenant_id: Optional[str] = None) -> List[Dict[str, Any]]` | yes | List recent runs from PostgreSQL. |
...
| `app.db` | CostBudget, get_async_session | no |
| `app.core.execution_context` | ExecutionContext, EvidenceSource | no |
| `app.evidence.capture` | capture_environment_evidence | no |
| `app.models.tenant` | WorkerRun | no |
| `app.policy.engine` | PolicyEngine | no |
| `app.schemas.response` | wrap_dict | no |
| `app.hoc.cus.general.L6_drivers.worker_write_service_async` | WorkerWriteServiceAsync | no |
...
| Import | Rule Broken | Required Fix | Line |
|--------|-------------|-------------|------|
| `from app.models.tenant import WorkerRun` | L2 MUST NOT import L7 models | Use L5 schemas or response models | 59 |
| `from app.hoc.cus.general.L6_drivers.worker_write_service_async import WorkerWriteServiceAsync` | L2 MUST NOT import L6 | Route through L3 → L5 → L6 | 63 |
