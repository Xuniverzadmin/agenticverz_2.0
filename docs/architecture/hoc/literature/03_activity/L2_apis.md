...
**Layer:** L2_api | **Domain:** activity | **Lines:** 2570
...
| `get_run_detail` | `(request: Request, run_id: str, session: AsyncSession = Depends(get_async_sessio` | yes | Get run detail (O3). Tenant isolation enforced. |
...
| `_get_runs_by_dimension_internal` | `(session: AsyncSession, tenant_id: str, dim: DimensionValue, state: RunState, li` | yes | Internal helper for dimension breakdown with HARDCODED state binding. |
...
| `get_risk_signals` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Ri` | yes | Returns aggregated risk signal counts. |
...
| `get_activity_metrics` | `(request: Request, session: AsyncSession = Depends(get_async_session_dep)) -> Me` | yes | Get aggregated activity metrics (V2). |
...
| `app.hoc.cus.activity.L5_engines.activity_facade` | get_activity_facade | no |
...
|--------|-------------|-------------|------|
| `from app.hoc.cus.activity.L5_engines.activity_facade import get_activity_facade` | L2 MUST NOT import L5 directly | Route through L3 adapter | 68 |
