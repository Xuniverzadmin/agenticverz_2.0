...
class AOSLoadTest:
    ...
    async def run_simulate_load(
        self, concurrency: int = 5, total_requests: int = 50
    ) -> LoadTestResult:
            ...
            async def bounded_request():
                async with sem:
                    return await self._make_request(
                        session, "POST", "/api/v1/runtime/simulate", json_data=payload
                    )
    ...
    async def run_capabilities_load(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async def bounded_request():
                async with sem:
                    return await self._make_request(
                        session, "GET", "/api/v1/runtime/capabilities"
                    )
    ...
    async def run_mixed_workload(
        self, duration_seconds: int = 30, concurrency: int = 10
    ) -> LoadTestResult:
        ...
        endpoints = [
            ...
            (
                ...
                {"plan": [{"skill": "llm_invoke", "params": {}}], "budget_cents": 100},
            ),
    ...
    def _calculate_stats(
        self, results: List[RequestResult], duration: float
    ) -> LoadTestResult:
...
async def main():
    ...
    parser.add_argument(
        "--requests", type=int, default=100, help="Total requests per test"
    )
    parser.add_argument(
        "--duration", type=int, default=30, help="Duration for mixed workload"
    )
    parser.add_argument(
        "--test",
        choices=["health", "simulate", "capabilities", "mixed", "all"],
        default="all",
    ...
    if args.test in ["simulate", "all"]:
        result = await tester.run_simulate_load(
            args.concurrency // 2, args.requests // 2
        )
