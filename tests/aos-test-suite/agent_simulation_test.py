...
class AgentSimulator:
    ...
    def create_synthetic_agents(self, count: int = 10) -> List[SyntheticAgent]:
        ...
        for i in range(count):
            ...
            print(f"  Created: {agent.name} ({agent.agent_type.value}) - {len(capabilities)} skills")
    ...
    def create_test_workflows(self) -> List[Workflow]:
        ...
        self.workflows = [
            ...
            Workflow(
                name="DataFetch",
                steps=[
                    WorkflowStep(SkillType.HTTP_CALL, {"url": "https://api.example.com/data"}, 0),
            ...
            Workflow(
                name="LLMAnalysis",
                steps=[
                    WorkflowStep(SkillType.LLM_INVOKE, {"prompt": "Analyze data", "model": "claude-3-sonnet"}, 5),
                ],
            ...
            Workflow(
                name="FullPipeline",
                steps=[
                    WorkflowStep(SkillType.HTTP_CALL, {"url": "https://api.example.com"}, 0),
                    WorkflowStep(SkillType.JSON_TRANSFORM, {"jq": "."}, 0),
                    WorkflowStep(SkillType.LLM_INVOKE, {"prompt": "Process"}, 5),
                    WorkflowStep(SkillType.WEBHOOK_SEND, {"url": "https://webhook.site/test"}, 0),
                ],
        ...
        for wf in self.workflows:
            ...
            print(f"  {wf.name}: {len(wf.steps)} steps, ~{total_cost}¢, ~{wf.expected_duration_ms}ms")
            ...
            async def worker(worker_id: int):
                ...
                while time.perf_counter() < end_time:
                    ...
                    if elapsed < ramp_up_seconds:
                        target = int(1 + (max_concurrency - 1) * (elapsed / ramp_up_seconds))
...
def print_results(title: str, results: Dict):
    ...
    if "agent_stats" in results:
        ...
        for agent in results["agent_stats"][:5]:  # Top 5
            print(f"    {agent['name']}: {agent['completed']} completed, {agent['failed']} failed")

async def main():
    parser = argparse.ArgumentParser(description="AOS Agent Simulation")
    parser.add_argument("--agents", type=int, default=10, help="Number of synthetic agents")
    parser.add_argument("--workflows", type=int, default=100, help="Number of workflows to run")
    parser.add_argument("--concurrency", type=int, default=10, help="Concurrent workflows")
    parser.add_argument("--stress", action="store_true", help="Run stress test")
    parser.add_argument("--stress-duration", type=int, default=30, help="Stress test duration")
