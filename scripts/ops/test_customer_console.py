...
def assert_shape(name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not result["ok"]:
        ...
        result["assertion_error"] = result.get("error", f"HTTP {result.get('status_code')}")
        return result

    try:
        if name == "pulse":
            assert "system_state" in body or "system_status" in body, "pulse missing system_state"
            ...
            expected = {"silent_churn", "policy_friction", "abandonment", "engagement_decay", "legal_only"}
            result["playbooks_configured"] = expected.issubset(set(playbook_ids))

        elif name == "infra":
            assert "db_connections_current" in body or "db" in body, "infra missing db info"
            assert "redis_memory_used_mb" in body or "redis" in body, "infra missing redis info"
...
def print_verbose(name: str, result: Dict[str, Any]):
    ...
    if name == "pulse" and result["ok"]:
        print(f"      Status: {body.get('system_state', body.get('system_status', 'unknown'))}")
        ...
        print(f"      Sample customer segments:")
        for c in body[:3]:
            delta = c.get('stickiness_delta', 1.0)
            trend = "↑" if delta > 1.1 else "↓" if delta < 0.9 else "→"
            print(f"        - {c.get('tenant_id', '?')[:20]}... | stickiness: {c.get('stickiness_7d', 0):.1f} (7d) {trend}")
        ...
        for c in body[:3]:
            ...
            print(f"        - {color}{risk.upper()}{RESET}: {c.get('tenant_id', '?')[:20]}...")
            print(f"          Reason: {c.get('primary_risk_reason', 'unknown')}")

    elif name == "playbooks" and result["ok"] and body:
        print(f"      Configured playbooks:")
        for p in body:
            risk = p.get('risk_level', 'unknown')
            color = RED if risk == 'critical' else YELLOW if risk == 'high' else RESET
            print(f"        - {BOLD}{p.get('id')}{RESET}: {p.get('name')} [{color}{risk}{RESET}]")

    elif name == "infra" and result["ok"]:
        db_pct = (body.get('db_storage_used_gb', 0) / max(body.get('db_storage_limit_gb', 1), 1)) * 100
        redis_pct = (body.get('redis_memory_used_mb', 0) / max(body.get('redis_memory_limit_mb', 1), 1)) * 100
        print(f"      DB: {body.get('db_storage_used_gb', 0):.2f}/{body.get('db_storage_limit_gb', 0):.0f} GB ({db_pct:.1f}%)")
        print(f"      Redis: {body.get('redis_memory_used_mb', 0):.1f}/{body.get('redis_memory_limit_mb', 0):.0f} MB ({redis_pct:.1f}%)")
        print(f"      Connections: {body.get('db_connections_current', 0)}/{body.get('db_connections_max', 0)}")
...
def main():
    parser = argparse.ArgumentParser(description="M24 Ops Console - Customer Console Test")
    ...
    all_ok = all(r.get("ok") and r.get("assertion") == "PASSED" for r in results.values())
    ...
    print(f"  {DIM}API: {BASE_URL}  |  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{RESET}\n")
    ...
    for name, r in results.items():
        ...
        print(f"  {status_icon} {name:20s} {latency:>8s}  {assertion_color}{assertion}{RESET}{count_info}")
    ...
    if empty_ops_events:
        ...
        print(f"  {DIM}At-risk customers appear after friction events are emitted:{RESET}")
