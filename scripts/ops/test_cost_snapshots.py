...
async def test_snapshot_computation(tenant_id: str):
    ...
    try:
        ...
        for row in result.fetchall():
            ...
            print(f"   {row.entity_type}: {entity} → ${row.total_cost_cents/100:.4f} ({row.request_count} requests)")
...
async def test_anomaly_evaluation(snapshot_id: str, tenant_id: str):
    ...
    try:
        ...
        for eval in evaluations:
            ...
            print(f"      {status} - {eval.deviation_pct:.1f}% deviation (threshold: {eval.threshold_pct}%)")
            if eval.triggered:
...
async def verify_db_state(tenant_id: str):
    ...
    try:
        ...
        print("\n📸 Snapshots:")
        for row in result.fetchall():
            print(f"   {row.id}: {row.status} ({row.records_processed} records, {row.computation_ms}ms)")
