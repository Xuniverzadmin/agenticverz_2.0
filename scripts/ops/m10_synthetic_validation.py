...
class UpstashValidator:
    ...
    async def inject_test_data(self) -> dict:
        ...
        try:
            ...
            msg_id = await self.client.xadd(stream_key, {
                'test_id': test_id,
                'timestamp': timestamp,
                'event': 'synthetic_injection'
            })
            results['records']['stream'] = {'key': stream_key, 'msg_id': msg_id.decode() if isinstance(msg_id, bytes) else str(msg_id)}
            logger.info(f"Injected stream record: {stream_key} -> {msg_id}")

            # 3. Add to a hash
            hash_key = f"{TEST_PREFIX}hash:stats"
            await self.client.hset(hash_key, mapping={
                'last_test_id': test_id,
                'last_timestamp': timestamp,
                'injection_count': await self.client.hincrby(hash_key, 'injection_count', 1)
            })
    ...
    while True:
        try:
            ...
            if results['overall_success']:
                neon_latency = results.get('neon', {}).get('validate', {}).get('checks', {}).get('write_latency_ms', 'N/A')
                redis_latency = results.get('redis', {}).get('validate', {}).get('checks', {}).get('ping_latency_ms', 'N/A')
                logger.info(f"  Neon write latency: {neon_latency}ms, Redis ping: {redis_latency}ms")
            else:
                errors = []
                for db in ['neon', 'redis']:
                    for phase in ['inject', 'validate', 'cleanup']:
                        phase_errors = results.get(db, {}).get(phase, {}).get('errors', [])
...
def main():
    ...
    parser.add_argument('--full', action='store_true', help='Run full inject/validate/cleanup cycle')
