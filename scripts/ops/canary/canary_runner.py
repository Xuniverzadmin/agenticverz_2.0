            ...
            if len(lines) >= 2:
                ...
                if len(parts) >= 2:
                    ...
                    if fstype in ('nfs', 'nfs4', 'cifs', 'glusterfs', 'ceph'):
                        log('WARN', f"Feature flags on network filesystem ({fstype}) - flock may not be reliable!")
                    return fstype
    except Exception:
        pass
    return "unknown"
...
def write_incident_fallback(report: 'CanaryReport', fallback_dir: Path = None) -> Optional[str]:
...
def toggle_flag(flag_name: str, enable: bool, env: str = "staging",
                dry_run: bool = False, create_backup: bool = True) -> Tuple[bool, str]:
    ...
    try:
        ...
        if dry_run:
            log('INFO', f"[DRY-RUN] Would set {flag_name}={enable} in {env} (current={current_value})")
            return True, "dry-run"
...
def run_smoke_script(script_path: str, timeout: int = 60) -> Tuple[int, str, str, float]:
...
def query_prometheus_range(prom_url: str, query: str, window_seconds: int = 300) -> Optional[float]:
    ...
    try:
        ...
        if data.get('status') != 'success':
            ...
            log('WARN', f"Range query failed, falling back to instant query")
...
def get_baseline_metrics(prom_url: str, metrics_config: Dict[str, str],
...
def get_current_metrics(prom_url: str, metrics_config: Dict[str, str]) -> Dict[str, float]:
...
def check_thresholds(baseline: Dict[str, float], current: Dict[str, float],
                     thresholds: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if 'golden_mismatch' in current:
        if current['golden_mismatch'] > baseline.get('golden_mismatch', 0):
            return True, f"golden_mismatch increased: {baseline.get('golden_mismatch', 0)} -> {current['golden_mismatch']}"
    ...
    if 'error_rate' in current and 'error_rate' in baseline:
        if baseline['error_rate'] > 0:
            delta_pct = ((current['error_rate'] - baseline['error_rate']) / baseline['error_rate']) * 100
            if delta_pct > error_delta_pct:
                return True, f"error_rate increased {delta_pct:.1f}% (threshold: {error_delta_pct}%)"
    ...
    if 'latency_p95' in current and 'latency_p95' in baseline:
        ...
        if delta > latency_delta_ms:
            return True, f"latency_p95 increased {delta:.1f}ms (threshold: {latency_delta_ms}ms)"

    return False, ""


def check_metric_uptime(prom_url: str, min_uptime_seconds: int = 300) -> Tuple[bool, float]:
    ...
    try:
        ...
        if uptime < min_uptime_seconds:
            log('WARN', f"Prometheus uptime {uptime:.0f}s < required {min_uptime_seconds}s")
            return False, uptime

        log('INFO', f"Prometheus uptime: {uptime:.0f}s (required: {min_uptime_seconds}s)")
        return True, uptime
...
def send_alert_with_retry(webhook_url: str, report: 'CanaryReport',
...
def execute_rollback(flags_to_disable: List[str], env: str = "staging",
...
def run_canary(config: Dict[str, Any], dry_run: bool = False,
               watch_seconds: int = 0) -> CanaryReport:
    ...
    if fstype not in ('unknown', 'ext4', 'ext3', 'xfs', 'btrfs', 'tmpfs'):
        log('WARN', f"Feature flags on {fstype} filesystem - atomic operations may be unreliable")
    ...
    smoke_script = smoke_cfg.get('script', str(PROJECT_ROOT / 'scripts/ops/canary_smoke_test.sh'))
    smoke_timeout = smoke_cfg.get('timeout_seconds', 60)
    report.smoke_script = smoke_script

    # Check if metric uptime guard is required
    require_metric_uptime = config.get('prometheus', {}).get('require_metric_uptime', True)
    ...
    try:
        ...
        wait_for_uptime = config.get('prometheus', {}).get('wait_for_metric_uptime', False)
        if require_metric_uptime and not dry_run:
            log('STEP', f"Checking Prometheus metric uptime (min: {baseline_window}s)...")
            ...
            if not is_ready and uptime >= 0:
                if wait_for_uptime:
                    ...
                    log('WARN', f"Prometheus uptime {uptime:.0f}s < required {baseline_window}s")
                    log('INFO', f"Waiting {wait_time}s for metrics to stabilize...")
                    time.sleep(wait_time)
                    is_ready, uptime = check_metric_uptime(prom_url, baseline_window)

                if not is_ready and uptime >= 0:
                    report.errors.append(f"Prometheus uptime {uptime:.0f}s < required {baseline_window}s")
                    report.success = False
                    log('ERROR', f"Aborting: metrics not stable (uptime {uptime:.0f}s < {baseline_window}s)")
                    return report

        # Step 1: Get baseline metrics with range averaging
        log('STEP', f"Collecting baseline metrics (avg over {baseline_window}s)...")
        report.metrics_baseline = get_baseline_metrics(prom_url, metrics_queries, baseline_window)
        ...
        if not dry_run:
            ...
            report.smoke_stdout = stdout[-2000:] if len(stdout) > 2000 else stdout  # Truncate
        ...
        if not report.rollback_triggered and not dry_run:
            ...
            for key in report.metrics_baseline:
                if key in report.metrics_current:
                    report.metrics_delta[key] = report.metrics_current[key] - report.metrics_baseline[key]
...
def main():
    ...
    parser.add_argument('--config', required=True, help='Path to canary config (YAML/JSON)')
    parser.add_argument('--report', default='scripts/ops/canary/reports/latest_report.json',
