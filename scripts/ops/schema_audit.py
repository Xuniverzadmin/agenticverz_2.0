    ...
    for table_full, constraints in REQUIRED_CONSTRAINTS.items():
        ...
        for constraint_name, constraint_type in constraints:
            if (constraint_name, constraint_type) not in existing:
                errors.append(f"Missing constraint: {table_full}.{constraint_name} ({constraint_type})")
...
def check_alembic_version(conn) -> list:
    ...
    if count == 0:
        ...
        errors.append(f"Alembic has multiple heads ({count}) - migration branching detected")
...
def run_audit() -> dict:
    ...
    results = {
        ...
        'database_url': os.getenv('DATABASE_URL', '').split('@')[-1],  # Hide credentials
        'checks': {},
        'errors': [],
        'passed': True,
    }
