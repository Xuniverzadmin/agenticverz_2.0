...
class RunCompletionTransaction:
    ...
    def _complete_trace(
        ...
        try:
            ...
            # TraceFacade.complete_trace is async, need to run in event loop
            loop = asyncio.new_event_loop()
            try:
                trace_success = loop.run_until_complete(
                    facade.complete_trace(
                        trace_id=trace_id,
                        status=run_status,
                    )
                )
            ...
            txn_result.trace_result = DomainResult(
                ...
                result_id=trace_id if trace_success else None,
